"""Deterministic decision node engine orchestration."""
    ...
    def apply_stage1(out: dict[str, Any], frame: Any) -> None:
        ...
        node_24_final = next(
            (
                ...
                if isinstance(n, dict) and str(n.get("node_id", "")) == "2.4"
            ),
            None,
        )
        ...
        try:
            ...
            out["trend_context"] = build_trend_context(
                frame,
                str(out.get("direction", "neutral")),
            )
    ...
    def apply_stage2(
        ...
        _node_90 = next(
            (
                x
                for x in _dt
                if isinstance(x, dict) and str(x.get("node_id", "")) == "9.0"
            ),
            None,
        )
