...
- CI（`.github/workflows/ci.yml`）目前在 Windows + Python 3.11 下执行安装/import 验证，并运行一组 package metadata、package marker imports、indicators package marker、security package marker、config path constants、crash diagnostics helper、safe filename helper、mask secret helper、形成中 K 线/数据源、刷新循环 warmup、刷新策略 helper、K-line 复权偏好 helper、A 股共享 helper、A 股涨跌停 helper、East Money quote page URL helper、East Money field enum helper、East Money quote API constants、East Money low-level client helpers、bar geometry primitives、trace node result helpers、signal context helpers、token counter helper、retry policy helper、validation message formatter、strategy filename registry、business-rule validators、schema structural validator、experience library renderer、QClaw relay helper、A 股/Tushare 无网络数据源、East Money quote 解析与 Baostock fallback helper、TradingView socket/error/connectivity/symbol lookup、market defaults、MT5 symbol availability、QClaw/Cursor route、CursorSdkClient bridge patch 合同、property invariants、non-live integration / non-e2e gate、provider override by model、KV prefix cache provider 判定、PushPlus mock 通知、API key configured helper、时间戳转换、timefmt epoch ms helper、CancelToken/OrchestratorEvent 基础线程工具、KlineBar 归一化、收盘等待/快照 warmup、ATR true range、ATR helper、EMA helper、K 线方向/几何特征、price tick/breakout entry 归一化、order opportunity alert helper、market features、Stage 1 支撑/阻力刷新、Stage 1 pattern routing、Stage 1/Stage 2 normalizer、Stage 1 策略文件别名、decision stance、decision panel UI 合同、decision tree helpers、decision continuity、decision nodes facade exports、PreflightDataGate / decision node judges property tests、支撑/阻力图表线与 overlay 价格精度、chart decision continuity overlay、chart fit view、chart skip-redraw、chart no-lines when not trading、prediction display formatting helper、Stage 2 UI payload merge helper、snapshot worker helper、status bar widget helper、validation debug dialog helper、flow bar widget helper、summary strip widget helper、sequence label item helper、candle item helper、toast overlay helper、model selector helper、widgets package exports、GUI package exports、orchestrator package exports、records package exports、demo package exports、GUI theme apply helper、GUI theme package exports、GUI theme design tokens、util package exports、DebugWidget key masking、token indicator thresholds、free chat reasoning resend、DeepSeekClient provider 参数合同、trade metrics / trade metrics validation、限价单 K1 新鲜度、cycle 枚举、响应提取、trace semantic/normalize checks、coherence validators、JSON repair/validator、lenient validation auto-fixes、MiMo 兼容、prompt txt 文件清单/prompt assembler、AI client factory 路由、SessionTokenLedger、provider quota/402 检测、validation retry/retry policy、分析历史增量定位、demo record/replay、§11 路由、决策编排、provider fallback、安全加密、settings round-trip 与 PendingWriter 脱敏/文件名安全相关的目标测试；目标 pytest 会通过 `pytest-cov` 输出 `pa_agent` 覆盖率报告但暂不设阈值，并额外运行 `pytest -m "not e2e and not live"` 作为非 live 非 e2e 回归门禁。Ruff 门禁覆盖应用入口与包入口文件组、notify 包入口、forming-bar/data-source、收盘等待/快照 warmup 测试、数据源工厂、A 股共享工具、AkShare/Tushare source、East Money client/source/Baostock fallback/quote API 常量/解析、数据层小文件组、刷新循环、A 股涨跌停辅助、TradingView source/connectivity、`tv_symbol_lookup`、`market_defaults`、`datetime_ts`、`kline_adjust`、`kline_features`、AI 基础叶子模块、AI schema/routing 辅助文件、AI token/signal 辅助文件、AI validation messages、AI experience renderer、AI business rules、AI decision node compatibility facade、QClaw relay 脚本、配置路径常量、验证重试编排、util 包入口/EventBus/日志/崩溃诊断、`structure_levels`、`chart_decision_overlay`、`ai_sidebar`、`prediction_format`、`stage2_payload`、`validation_debug_dialog`、util 小工具组、`trade_metrics`、`ai/prompts`、`cycle_enums`、`response_extract`、`mimo_compat`、`client_factory`、`session_ledger`、`cursor_sdk_client`、`provider_errors`、`retry_policy`、`records`、`demo`、`ProviderSyncService`、`security`、`indicators`、`gui/theme`、测试包入口与子包入口、目标测试文件、`tests/property`、`tests/integration`、`tests/e2e` 及 `tests/fixtures`。尚未启用 live/e2e、全仓 Ruff、Black 或覆盖率阈值门禁。
...
- 第一百七十七轮 L7 补充：新增 `tests/unit/test_ashare_common.py` 并纳入目标 pytest 与 focused Ruff，覆盖 A 股代码归一化/指数 API symbol、交易时段/日线 live 判定、quote 手数转股数、forming row quote 合并和 4h resample；本轮不修改 A 股共享 helper、数据源调用或 pandas/网络路径。
- 第一百七十八轮 L7 补充：新增 `tests/unit/test_prediction_format.py` 并纳入目标 pytest 与 focused Ruff，覆盖下一根 K 线/周期预测展示概率行格式化、缺失概率 fallback、dominant direction 数值判定和非法值忽略；本轮不修改 prediction display helper、面板 UI、颜色常量或任何用户可见文本。
- 第一百七十九轮 L7 补充：新增 `tests/unit/test_stage2_payload.py` 并纳入目标 pytest 与 focused Ruff，覆盖 Stage 2 UI payload 非 dict fallback、顶层预测字段合并、归一化前 deepcopy 和 `skip_next_bar` 预移除；本轮不修改 Stage 2 payload helper、normalizer、UI 面板或展示文案。
- 第一百八十轮 L7 补充：新增 `tests/unit/test_status_bar.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `EnhancedStatusBar` 的 message 兼容别名、progress value/label、颜色 fallback 与 TPS 显示/隐藏；本轮不修改 status bar widget、样式颜色、布局或主窗口集成。
- 第一百八十一轮 L7 补充：新增 `tests/unit/test_validation_debug_dialog.py` 并纳入目标 pytest 与 focused Ruff，覆盖验证失败调试弹窗的标题/尺寸、summary label、只读正文、按钮文案、blank summary 省略和复制全部逻辑；本轮不修改 dialog helper、弹窗文案、剪贴板调用或主窗口触发路径。
- 第一百八十二轮 L7 补充：新增 `tests/unit/test_flow_bar.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `FlowBar` 默认步骤/默认 caption、step status/caption 更新、越界更新忽略和 `reset_all()` 复位；本轮不修改 flow bar widget、默认步骤文案、颜色表、布局或主窗口集成。
- 第一百八十三轮 L7 补充：新增 `tests/unit/test_summary_strip.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `SummaryStrip` 默认 metric cards、匹配 key 更新、未知 key 忽略、`reset()` 默认值恢复和 `_relayout()` grid 合同；本轮不修改 summary strip widget、默认指标文案、字体/样式或主窗口集成。
- 第一百八十四轮 L7 补充：新增 `tests/unit/test_seq_label_item.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `SeqLabelItem` 序号文本、默认/形成中颜色、字体字号、anchor 与坐标合同；本轮不修改 seq label widget、形成中颜色、默认颜色、字体、anchor、位置或图表集成。
- 第一百八十五轮 L7 补充：新增 `tests/unit/test_candle_item.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `CandleItem` body bounds、doji 最小高度、closed/forming bounding rect 宽度与 `update_bar()` 状态切换合同；本轮不修改 candle item widget、蜡烛颜色、body/wick 绘制、形成中样式、bounding rect 公式或图表集成。
- 第一百八十六轮 L7 补充：新增 `tests/unit/test_toast_overlay.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `_ToastLabel` 换行/居中/样式、`ToastOverlay.show_message()` 列表与布局追加、bottom-right 定位、`_dismiss()` 移除和无 parent reposition no-op 合同；本轮不修改 toast widget、自动关闭计时器、样式、定位公式或主窗口集成。
- 第一百八十七轮 L7 补充：新增 `tests/unit/test_model_selector.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `ModelDropdown` 分组选项重建、当前模型选中态、selected callback 信号，以及 `ModelSelector` model name/groups 更新和选择信号；本轮不修改 model selector widget、按钮文案、dropdown 定位、样式或主窗口集成。
- 第一百八十八轮 L7 补充：新增 `tests/unit/test_snapshot_worker.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `SnapshotFetchWorker.run()` 对 `latest_snapshot(n_bars)` 的参数传递、成功 `bars_ready` 信号和异常 `failed` 信号；本轮不修改 snapshot worker、线程启动方式、数据源调用或失败日志语义。
- 第一百八十九轮 L7 补充：新增 `tests/unit/test_widgets_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.gui.widgets` 的 `__all__` 顺序和公开 widget 类导出绑定；本轮不修改 widgets 包入口、公开类集合、widget 初始化路径或主窗口集成。
- 第一百九十轮 L7 补充：新增 `tests/unit/test_gui_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.gui` 的 `__all__` 顺序和 GUI 顶层公开类导出绑定；本轮不修改 GUI 包入口、公开类集合、导出名称、初始化路径或主窗口集成。
- 第一百九十一轮 L7 补充：新增 `tests/unit/test_orchestrator_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.orchestrator` 的 `__all__` 顺序和 `FreeChatSession`/`TwoStageOrchestrator` 导出绑定；本轮不修改 orchestrator 包入口、公开类集合、编排器运行逻辑或调用路径。
- 第一百九十二轮 L7 补充：新增 `tests/unit/test_records_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.records` 的 `__all__` 顺序和 records schema / reader 公开类导出绑定；本轮不修改 records 包入口、schema 模型、reader 行为、持久化路径或记录格式。
- 第一百九十三轮 L7 补充：新增 `tests/unit/test_demo_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.demo` 的 `__all__` 顺序和 demo replay / record loader 公开对象导出绑定；本轮不修改 demo 包入口、回放定时逻辑、记录加载、随机选择或可播放判定。
- 第一百九十四轮 L7 补充：新增 `tests/unit/test_gui_theme_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.gui.theme` 的 `__all__` 顺序和 `apply_theme` 导出绑定；本轮不修改 theme 包入口、QSS 读取、Fusion 样式设置或应用主题逻辑。
- 第一百九十五轮 L7 补充：新增 `tests/unit/test_util_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.util` 的 `__all__` 顺序和 `CancelToken`/`EventBus`/`OrchestratorEvent`/日志配置函数导出绑定；本轮不修改 util 包入口、Qt 信号、线程取消语义、日志 handler 配置或 API key 脱敏逻辑。
- 第一百九十六轮 L7 补充：新增 `tests/unit/test_decision_nodes_exports.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.ai.decision_nodes` compatibility facade 的 `__all__` 顺序和 27 个公开阈值/engine/preflight/judge/router/trace helper 导出绑定；本轮不修改 decision node facade、判定阈值、judge 逻辑、router 行为或 override/trace helper。
- 第一百九十七轮 L7 补充：新增 `tests/unit/test_package_metadata.py` 并纳入目标 pytest 与 focused Ruff，覆盖顶层 `pa_agent.__version__` 与 `pyproject.toml` project version 的一致性以及包 docstring 合同；本轮不修改顶层包入口、项目版本、打包配置或启动入口。
- 第一百九十八轮 L7 补充：新增 `tests/unit/test_config_paths.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.config.paths` 中项目根、提示词目录、运行目录和关键文件路径常量的派生关系；本轮不修改路径常量、目录创建、配置读写或日志/记录落盘路径。
- 第一百九十九轮 L7 补充：新增 `tests/unit/test_package_markers.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.ai`、`pa_agent.config`、`pa_agent.data`、`pa_agent.notify` 轻量包入口可导入、无 `__all__` 和既有 package docstring 合同；本轮不修改包入口、子模块导入路径、公开 API 或通知/配置/数据层逻辑。
- 第二百轮 L7 补充：新增 `tests/unit/test_crash_diagnostics.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.util.crash_diagnostics._log_file_handler_attached()` 对匹配/缺失/不匹配 `RotatingFileHandler` 的判定；本轮不启用 faulthandler、不写真实 crash log、不修改 crash diagnostics、日志 handler 配置或 crash/log 路径常量。
- 第二百零一轮 L7 补充：新增 `tests/unit/test_safe_filename.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `sanitize_filename_component()` 对非法字符、路径分隔符、空结果 fallback、首尾点/空格/短横线、Windows 保留名和正常值保持的边界处理；本轮不修改文件名清洗规则、记录命名、日志命名或落盘路径。
- 第二百零二轮 L7 补充：新增 `tests/unit/test_mask_secret.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `mask_secret()` 对空串、短串、四字符边界、普通密钥和 Unicode 字符串的掩码输出；本轮不修改脱敏规则、日志 formatter、PendingWriter 记录脱敏或任何密钥持久化逻辑。
- 第二百零三轮 L7 补充：新增 `tests/unit/test_ema.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `ema_full()` warmup/seed/smoothing、`period=1`、非法周期、`make_ema_state()` 初始状态和 `state_after()` 增量最终状态；本轮不修改 EMA 公式、增量状态结构、ATR、snapshot warmup 或任何指标计算运行逻辑。
- 第二百零四轮 L7 补充：新增 `tests/unit/test_atr.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `atr_full()` warmup/seed/Wilder smoothing、`period=1`、非法周期、输入长度不一致、`make_atr_state()` 初始状态和 `state_after_atr()` 增量最终状态；本轮不修改 ATR 公式、true range 规则、EMA、snapshot warmup 或任何指标计算运行逻辑。
- 第二百零五轮 L7 补充：新增 `tests/unit/test_indicators_package.py` 并纳入目标 pytest 与 focused Ruff，覆盖 `pa_agent.indicators` 轻量包入口可导入、无 `__all__`、不重导出 ATR/EMA helper 和既有 package docstring 合同；本轮不修改 indicators 包入口、ATR/EMA 公式、指标导入路径或任何运行逻辑。
