...
- Prompt ID 解耦方案：[`docs/prompt_id_decoupling_plan.md`](./docs/prompt_id_decoupling_plan.md)，
  ...
  离线/真实 Provider 退出门禁已完成，M5.0-M5.5 与 29/29 最终审计已收口。
...
- M5.3 Stage 1/Base 迁移诊断：
  [`docs/diagnostics/prompt_m5_stage_base_migration_2026-07-29.md`](./docs/diagnostics/prompt_m5_stage_base_migration_2026-07-29.md)，
  固定 5 个 Binary Decision 直接依赖模板的双口径正文与组装摘要证据。
- M5.4 Router 语义对迁移诊断：
...
18. **L5 经验库评估当前进度**：经验目录当前只有占位文件，禁止把合成 fixture 的指标当作
    ...
    或证据不明必须 defer；提交 `dfb0188` 已由 GitHub Actions run `30159406614` 在
...
20. **L2 Prompt 兼容观察当前进度**：TemplateStore、TemplateContext、严格变量渲染和
    ...
    `route_strategy_files()` 与私有 filename helper 只做 Catalog 投影；M2.2 已将 shared
    system 与 Stage 1 builder 切换为原子 ID loader，旧 `*_TXT_FILES`/legacy fallback 与
    Prompt golden 不变。M2.3 已将 Stage 2 builder 与 TemplateContext 切换为 ID，并保留
    filename 公共参数和双合同投影。M3.1 已让 `AnalysisRecord` 双写
    `strategy_prompt_ids_used`/`strategy_files_used`，已知旧 filename 在内存中补全 ID，
    ...
    始终由 router 重建。M4.2 已从运行时模板、增量/prefix-chain carryover 和 Stage 2 compact
