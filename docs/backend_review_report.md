...
- L3 第 238 轮已新增 PyQt-free `orchestrator/pipeline/` 状态/步骤协议、显式终态和
  ...
  `RouteStep`，opt-in pipeline sequence 为
  `Stage1Step -> RouteStep -> legacy_stage2_persist`；后者仍承接 Stage 2/Persist 兼容尾步骤。
  `TwoStageOrchestrator.submit()` 默认行为保持不变。Stage 1 的 happy/retry/network/validation/
  cancel/incremental，以及 RouteStep 的 callable/object router、策略顺序、经验限制、空经验库、
  `current_bars`、Stage 2 前取消和 `route_failed` 已有测试证据，完整尾部步骤化仍待完成。
...
| L3 | 引入 Pipeline Builder（Stage1Step/RouteStep 已交付） | `orchestrator/pipeline/`、`orchestrator/two_stage.py`、`tests/unit/test_pipeline.py`、`tests/integration/test_stage1_pipeline_step.py`、`tests/integration/test_route_pipeline_step.py` | state 已承载阶段 payload、usage、route 输出、持久化意图和安全摘要；opt-in sequence 已为 `Stage1Step -> RouteStep -> legacy_stage2_persist`；Stage 2/Persist 仍需逐步替代兼容尾步骤并验证旧/新事件与记录等价，默认 `submit()` 保持兼容 |
...
1. **继续推进 L3 等价 harness**：在已交付 `Stage1Step` 和 `RouteStep` 的基础上实现真实
   Stage 2/Persist steps，结合现有测试与真实 Provider runner、JSONL 事件重放验证 Pipeline
