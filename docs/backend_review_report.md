...
- L3 第 238 轮已新增 PyQt-free `orchestrator/pipeline/` 状态/步骤协议、显式终态和
  `LegacySubmitStep` compatibility adapter；本轮 Task 5 又补齐 Stage 1/Stage 2 payload、usage、
  route 输出、`PersistenceIntent`/partial reason、route/persist 终态映射和不暴露 payload 的安全
  摘要，且不改变 `AnalysisRecord` schema。`TwoStageOrchestrator.submit()` 默认行为保持不变，
...
| L3 | 引入 Pipeline Builder（PipelineState foundation 已交付） | `orchestrator/pipeline/`、`orchestrator/two_stage.py`、`tests/unit/test_pipeline.py` | state 已承载阶段 payload、usage、route 输出、持久化意图和安全摘要；route/persist 终态映射已补齐；后续仍需用真实 Stage 1/route/Stage 2/persist steps 逐步替代 legacy wrapper，并验证旧/新事件与记录等价，默认 `submit()` 保持兼容 |
...
1. **继续推进 L3 等价 harness**：在 PipelineState foundation 之上实现真实 Stage 1/route/Stage 2/
   persist steps，结合真实 Provider runner、JSONL 事件重放验证 Pipeline state/step 与旧
   `submit()` 的 record/事件等价；当前默认 `submit()` 仍是兼容路径。
