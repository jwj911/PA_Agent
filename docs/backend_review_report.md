...
- L5 已接入 K 线几何相似度；本轮新增版本化脱敏评估合同、离线
  ...
  `success|failure` 最小化导入、Key 二次脱敏和 digest 去重。真实 scan 仅 1 eligible、
  1 partial，经验目录仍为空，不能据此判断真实交易结构检索质量，也不应调整线上权重。
...
| L5 | 经验库升级（数据/评估管道完成，真实标注待输入） | `records/experience_reader.py`、`records/experience_similarity.py`、`records/experience_curation.py`、`records/experience_eval_pipeline.py` | Stage 2 先按 pattern + direction 排序，再以 K 线几何相似度打破同分；completed records 只能经人工 outcome 导入；opaque 标注、固定 split 和指标报告已具备，当前样本不足 |
...
1. **等待人工 outcome 后运行 L5 离线评估**：record curation 与安全导出/标注/报告管道已
   ...
   success/failure，补足至少两个 instrument group，再生成 Recall/NDCG、fallback 和
