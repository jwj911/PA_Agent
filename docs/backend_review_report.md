...
- L4 的热路径代码优化已完成；`pa-agent.performance.v1` 报告、p50/p95 budget、10%
  ...
  相同，但失败项在不同亚毫秒操作间漂移。当前 `l4.synthetic.v2` 改为按操作成本批量执行后
  折算单次耗时，保留原 10% 阈值和绝对预算，并使用独立 cache；本地九项通过，仍待 hosted v2
  首次建基线和第二次同参对照。
...
4. **完成 L4 v2 hosted 对照与 L1 扩展治理**：L4 v1 已证明 restore、10% 阻断和失败不覆盖
   baseline，但亚毫秒失败项漂移；`l4.synthetic.v2` 已使用批量折算采样和独立 cache，下一步
   先建立 hosted v2 baseline，再同参数完成第二次 p95 对照；L1 已完成固定样例的
   versioned/legacy 扩展兼容观察，
...
v2 `iterations=30`、`warmups=5` 建立 baseline，第二次同参数运行验证 cache restore、p95
regression 和 JSON artifact；v1 的三次证据保留用于说明门禁链路和采样修正原因，不作为 v2
性能 baseline。该权限仅用于触发/观察 workflow，不应导致 workflow 或日志扩大密钥、Provider
或 benchmark 敏感数据暴露。
