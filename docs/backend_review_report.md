...
`RouteStep`，Task 8 已交付真实 `Stage2Step`，Task 9 已交付真实 `PersistStep`，opt-in pipeline
sequence 为 `Stage1Step -> RouteStep -> Stage2Step -> PersistStep`。PersistStep 集中 full/partial
record assembly/write，前置终态以 `persistence_pending` 交给唯一一次 PersistStep 写入，并以
`PendingWriter.last_write_succeeded` 识别磁盘失败；full 写入成功后才发 `RecordSaved`，partial
或失败不发成功事件。`TwoStageOrchestrator.submit()` 默认行为保持不变。Stage 1 的
...
的 full/partial/insufficient-data/disk failure/重复保存防护均已有测试证据；Pipeline feature
flag、稳定观察周期和 GUI/headless 全链路等价仍待完成。
...
| L3 | 引入 Pipeline Builder（四个真实步骤已交付） | `orchestrator/pipeline/`、`orchestrator/two_stage.py`、`tests/unit/test_pipeline.py`、`tests/integration/test_stage1_pipeline_step.py`、`tests/integration/test_route_pipeline_step.py`、`tests/integration/test_stage2_pipeline_step.py`、`tests/integration/test_persist_pipeline_step.py` | state 已承载阶段 payload、usage、route 输出、Stage 2 flags、持久化意图、partial reason 和 `persistence_pending`；opt-in sequence 已为 `Stage1Step -> RouteStep -> Stage2Step -> PersistStep`；PersistStep 负责 full/partial record assembly/write、`RecordSaved` ordering 和 disk failure，`PendingWriter.last_write_succeeded` 提供写入结果；feature flag、稳定观察周期和 GUI/headless 全链路等价仍待完成，默认 `submit()` 保持兼容 |
...
1. **继续推进 L3 等价 harness**：在四个真实步骤已交付的基础上，完成 Pipeline feature flag
   的完整观察周期，结合现有测试、真实 Provider runner 和 JSONL 事件重放验证 Pipeline
   state/step 与旧 `submit()` 的 record/事件等价；当前默认 `submit()` 仍是兼容路径。
