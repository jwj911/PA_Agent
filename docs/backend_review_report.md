...
`orchestrator.pipeline_builder_enabled`、Settings round-trip/旧配置默认迁移、
...
的 full/partial/insufficient-data/disk failure/重复保存防护均已有测试证据；Task 10 的测试和
  ...
  fixture rollout 对照，覆盖 record、事件、prompt、流式内容、策略文件和 writer 边界；
  2026-07-23/24 又完成 3 个独立真实 legacy/Pipeline pair，6 个单体校验和 3 个成对校验全部
  `valid=true`，累计 18 个 artifact 文件密钥扫描 0 命中。默认 flag 已切换为 `true`，缺少
  字段的旧配置也采用 Pipeline；显式 `false` 保留 legacy 回滚。显式 live harness 的
...
- L5 已接入 K 线几何相似度；本轮新增版本化脱敏评估合同、离线
  ...
  `success|failure` 最小化导入、Key 二次脱敏和 digest 去重；脱敏 readiness preflight
  现可机器判定 export/evaluation 前置条件。真实 scan 仅 1 eligible、1 partial，经验目录
  仍为空；当前 blocker 为缺 salt、无案例和无 annotations，不能据此判断真实交易结构检索
  质量，也不应调整线上权重。
...
- L6 已完成 `AppEvent`/`EventSink`、`bootstrap_headless()`、共享 core/GUI bootstrap 边界，
  ...
  独立 Python 子进程重放证据。真实 Provider 成功主路径和三轮稳定观察已收敛；必须显式确认
  且只读环境变量凭据的
  ...
  合同；完整步骤见 `docs/live_observation_runbook.md`。真实成功 pair 不替代
  final/partial/cancel/failure 固定 fixture 矩阵。
...
gate short-circuit、取消和持久化失败只记录稳定分类或异常类型。三轮稳定观察完成后，
`orchestrator.pipeline_builder_enabled=true` 成为默认；显式 flag-off 继续走 legacy
`submit()`，不改变事件顺序、retry/cancel 语义或 final/partial record。默认路径继续使用该
日志做持续观察；出现未解释偏差时先回滚。`pipeline.timing` 还记录 Stage 2 启动前的
Stage 1/Route 边界耗时。
...
| L5 | 经验库升级（数据/评估管道完成，真实标注待输入） | `records/experience_reader.py`、`records/experience_similarity.py`、`records/experience_curation.py`、`records/experience_eval_pipeline.py` | Stage 2 先按 pattern + direction 排序，再以 K 线几何相似度打破同分；completed records 只能经人工 outcome 导入；readiness、opaque 标注、固定 split 和指标报告已具备，当前样本不足 |
...
1. **等待人工 outcome 后运行 L5 离线评估**：record curation 与安全导出/标注/报告管道已
   ...
   success/failure，补足至少两个 instrument group，使 export/evaluation preflight 通过，
   再生成 Recall/NDCG、fallback 和 ranking stability 报告。
2. **持续观察 L3/L6 默认路径**：Pipeline 已在三轮真实 pair 后默认启用；继续观察 lifecycle、
   terminal 和 record 指标，出现未解释偏差时显式回滚 `false`，暂不删除 legacy facade。
3. **维护 L4 持续观察并完成 L1 扩展治理**：L4 v1 已证明 restore、10% 阻断和失败不覆盖
...
4. **维护 L2 兼容证据**：下线策略/CI 门禁已固定，继续收集 fallback 零命中和 golden 等价
