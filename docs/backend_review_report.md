...
- L6 已完成 `AppEvent`/`EventSink`、`bootstrap_headless()`、共享 core/GUI bootstrap 边界，
  ...
  `HeadlessAnalysisAdapter`；GUI/headless 全链路 record 等价、真实 Provider 环境验证和事件
  envelope 版本契约仍未收敛。
...
1. **继续推进 L3 rollout evidence**：在四个真实步骤和 Task 10 flag 接线/矩阵测试已交付的基础上，
   保持 `orchestrator.pipeline_builder_enabled=false` 为默认，进行受控 flag-on 真实稳定观察周期，
   结合真实 Provider runner 和 JSONL 事件重放验证 Pipeline state/step 与旧 `submit()` 的
   record/事件等价；随后补齐 GUI/headless final/partial/cancel/failure 全链路 evidence，满足后
   才评估启用默认 flag。
