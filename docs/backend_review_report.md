...
- L4 的热路径代码优化已完成；本轮新增 `pa-agent.performance.v1` 固定 synthetic benchmark、
  p50/p95 budget 和 10% regression 判定，生成当前 Windows/Python 报告；CI/夜间持续回归和
  同环境 baseline 维护仍待完成。
...
4. **完成 L4 持续回归接线与 L1 扩展治理**：L4 固定 fixture 的 p50/p95 runner 和当前报告
   已建立，下一步接入 CI/夜间任务并维护同环境 baseline；L1 已完成固定样例的
   versioned/legacy 扩展兼容观察，下一步只观察真实已安装扩展和长期版本策略；不凭感觉修改
   热路径。
