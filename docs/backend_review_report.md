...
> 状态更新：2026-07-23。R1-R8、M1-M10 及 L1-L6 的实际进度以
...
### 1.1 当前进度校正（2026-07-22）

- L1 数据源/Provider 注册表已完成第二阶段基础，未知数据源配置已安全回退并持久化规范化值；
  第 236 轮又补齐了规范化 key、replace/unregister、priority 稳定性、并发读写和 lazy-import
  测试；前轮补齐已安装 entry point 发现、registrar 契约和扩展失败隔离，本轮又以 5 轮
  外部风格样例观察 `pa-agent.registry-extension.v1`、legacy callable registrar、settings
  注入、matcher 和 builder；当前仍保留旧入口并继续观察真实已安装扩展。下线策略已机器化：
  legacy callable 当前 `retain`，最早 `0.3.0` 且具备 `v0.2.0` tag、inventory 和迁移报告后才
  评估删除。
- L2 已完成 prompt 文件顺序、阶段边界和 Spike/Climax 硬约束的合同化基线；第 230 轮新增
  ...
  本轮又完成 5 轮固定 fixture 的 TemplateStore/旧 loader system、Stage 1、Stage 2 和
  continuation 字节等价观察；旧 loader/fallback 当前按政策 `retain`，最早 `0.3.0` 且具备
  `v0.2.0` tag、fallback 零命中和 golden 报告后才评估删除。
- L3 第 238 轮已新增 PyQt-free `orchestrator/pipeline/` 状态/步骤协议、显式终态和
  `LegacySubmitStep` compatibility adapter；Task 5 又补齐 Stage 1/Stage 2 payload、usage、
  route 输出、`PersistenceIntent`/partial reason、route/persist 终态映射和不暴露 payload 的安全
摘要，且不改变 `AnalysisRecord` schema。Task 6 已交付真实 `Stage1Step`，Task 7 已交付真实
`RouteStep`，Task 8 已交付真实 `Stage2Step`，Task 9 已交付真实 `PersistStep`，Task 10 已接入
默认关闭的 `orchestrator.pipeline_builder_enabled`、Settings round-trip/旧配置 legacy 默认、
flag-off/flag-on `submit()` 路由、完整终态矩阵和 Qt-free headless/GUI adapter equivalence 测试；
flag-on pipeline sequence 为 `Stage1Step -> RouteStep -> Stage2Step -> PersistStep`。PersistStep
集中 full/partial record assembly/write，前置终态以 `persistence_pending` 交给唯一一次 PersistStep
写入，并以 `PendingWriter.last_write_succeeded` 识别磁盘失败；full 写入成功后才发 `RecordSaved`，
partial 或失败不发成功事件。`TwoStageOrchestrator.submit()` 默认行为仍为 legacy。Stage 1 的
happy/retry/network/validation/cancel/incremental、RouteStep 的 callable/object router/策略
...
的 full/partial/insufficient-data/disk failure/重复保存防护均已有测试证据；Task 10 的测试和
  ...
  完成，之后才评估启用默认 flag；新增显式 live harness 的
  `--pipeline-builder-enabled` 仅对本次授权运行打开 Pipeline，未传参数保持 legacy。
- L5 已接入 K 线几何相似度；本轮新增版本化脱敏评估合同、离线
  `Recall@K`/`NDCG@K`/fallback/stability scorer 和 instrument-grouped
  `pa-agent.experience-split.v1` 固定切分/dataset digest；又新增 HMAC opaque catalog、
  人工标注门禁、leave-one-out legacy/similarity 对照和 `pa-agent.experience-eval-report.v1`
  报告 CLI。经验目录仍为空，不能据此判断真实交易结构检索质量，也不应调整线上权重。
- L4 的热路径代码优化已完成；`pa-agent.performance.v1` 报告、p50/p95 budget、10%
  regression 判定和 hosted cache 已接线。v1 run `29930306551`/`29974597115` 均恢复首次
  run `29923921295` 的 baseline 并正确阻断，失败未覆盖成功 baseline；三次 runner image
  相同，但失败项在不同亚毫秒操作间漂移。`l4.synthetic.v2` 改为按操作成本批量执行后折算
  单次耗时，保留原 10% 阈值和绝对预算，并使用独立 cache；run `29975410917`/
  `29975592352` 已完成 hosted v2 建基线和同参 restore 对照，九项全部通过，最大正回退
  `+5.12%`，L4 进入每日持续观察。
- L6 已完成 `AppEvent`/`EventSink`、`bootstrap_headless()`、共享 core/GUI bootstrap 边界，
  ...
  correlation 事件输出；本轮补齐公开 `HeadlessAnalysisAdapter` 的阶段回调合同、真实
  GUI `_AnalysisWorker` 与 headless adapter 的 final/partial/cancel/failure fixture 等价，
  并固定 `pa-agent.event.v1` envelope；本轮又补齐严格 `expected_correlation_id` replay 和
  独立 Python 子进程重放证据。真实 Provider 环境观察、真实运行 record/事件证据和
  record/event 完整等价仍未收敛；本轮新增必须显式确认且只读环境变量凭据的
  `tools/run_live_headless_observation.py`，不接入常规/夜间 CI；又新增
  `tools/validate_live_observation.py` 离线核对 summary/event/record 自洽性，以及
  `tools/compare_live_observations.py` 比较 legacy/Pipeline 的终态、事件和 shape-only record
  合同；完整步骤见 `docs/live_observation_runbook.md`。当前仍缺真实 `valid=true` pair，
  工具合同不替代真实 Provider 或 GUI/headless 等价证据。
...
Pipeline enabled 路径现在以 `trace_id` 关联一次执行，并提供四类结构化日志事件：
`pipeline.lifecycle`、`pipeline.event`、`pipeline.step` 和 `pipeline.timing`。字段以 `pipeline_step`、
...
不得据此启用默认 flag。`pipeline.timing` 还记录 Stage 2 启动前的 Stage 1/Route 边界耗时。
Task 11 已同步生命周期日志业务代码、聚焦测试和项目文档/规格；
验证通过并已按流程纳入原子提交/推送。
...
| 编号 | 任务 | 目标文件 | 预期收益 |
|---|---|---|---|
| L1 | Provider/数据源注册表与扩展契约（固定样例兼容观察完成，进入真实扩展观察期） | `pa_agent/extensions.py`、`ai/client_factory.py`、`ai/provider_registry.py`、`data/factory.py`、`data/registry.py`、`tests/unit/test_extensions.py`、`tests/integration/test_l1_extension_compatibility_observation.py` | AI client 与数据源支持规格注册、优先级/延迟 builder、运行时扩展、entry point registrar、`pa-agent.registry-extension.v1`、legacy registrar 兼容、失败隔离和生命周期/并发/lazy-import 证据；真实已安装扩展集合和长期版本策略仍待观察，Provider 同步仍由现有 service 负责 |
| L2 | Prompt 模板引擎化 | `prompt_engineering/`、`ai/prompt_assembler.py` | 使用 Jinja2 或结构化模板，支持热更新 |
| L3 | 引入 Pipeline Builder（Task 10 已完成 rollout 准备，默认 legacy） | `orchestrator/pipeline/`、`orchestrator/two_stage.py`、`pa_agent/config/orchestrator.py`、`tests/unit/test_pipeline.py`、`tests/unit/test_settings_round_trip.py`、`tests/integration/test_stage1_pipeline_step.py`、`tests/integration/test_route_pipeline_step.py`、`tests/integration/test_stage2_pipeline_step.py`、`tests/integration/test_persist_pipeline_step.py`、`tests/integration/test_task10_pipeline_rollout.py` | state 已承载阶段 payload、usage、route 输出、Stage 2 flags、持久化意图、partial reason 和 `persistence_pending`；flag-on sequence 已为 `Stage1Step -> RouteStep -> Stage2Step -> PersistStep`；`orchestrator.pipeline_builder_enabled` 默认 `false`，旧配置默认 legacy；Task 10 已有完整终态矩阵和 Qt-free adapter equivalence，CI target/config module 已同步；真实稳定观察周期和 GUI/headless 全链路等价仍待完成，默认 `submit()` 保持 legacy |
...
| L6 | 无 GUI 运行支持（headless 第一切片与公开 adapter 完成） | `pa_agent/headless.py`、`util/events.py`、`util/event_sink.py`、`util/event_bus.py`、`app_context.py`、`cli.py`、`records/pending_writer.py` | 默认 dry-run 保持无网络；显式 `--run/--execute` 通过公开 `HeadlessAnalysisAdapter` 复用 `TwoStageOrchestrator` 写入 final/partial record，并输出 correlation JSONL 事件；后续 GUI/headless 等价、真实 Provider 验证和事件 envelope 版本契约 |
...
1. **完成 L6 真实环境收口**：在 fixture 等价、`pa-agent.event.v1` envelope、严格跨进程
   replay contract、显式 live harness、单体/成对 validator 基础上，按
   `docs/live_observation_runbook.md` 取得至少一个真实 `valid=true` pair。
2. **推进 L3 rollout evidence**：保持 `orchestrator.pipeline_builder_enabled=false` 为默认，
   使用显式 live harness 分别运行 legacy 和 flag-on，以成对工具验证控制流和记录结构，
   满足后才评估启用默认 flag。
3. **等待数据后运行 L5 离线评估**：安全导出/标注/报告管道已建立；下一步按
   `docs/experience_evaluation_runbook.md` 导入真实案例和人工标签，生成 Recall/NDCG、
   fallback 和 ranking stability 报告。
4. **维护 L4 持续观察并完成 L1 扩展治理**：L4 v1 已证明 restore、10% 阻断和失败不覆盖
   baseline，v2 run `29975410917`/`29975592352` 已完成稳健采样的 hosted 建基线和同参
   p95 对照；后续由每日 schedule 观察。L1 下线策略/CI 门禁已固定，下一步只收集真实安装扩展
   inventory；不满足版本/tag/迁移证据时继续 retain。
5. **维护 L2 兼容证据**：下线策略/CI 门禁已固定，继续收集 fallback 零命中和 golden 等价
   报告；不满足版本/tag 条件时继续 retain。

L4 hosted v2 已以 `iterations=30`、`warmups=5` 完成 baseline 与第二次 cache restore/p95
对照。后续手动运行的网页入口要求仓库 Write 权限；CLI/API 入口使用仅限 `PA_Agent` 的
Fine-grained PAT，Repository `Actions: Read and write`。v1 三次证据仅说明门禁链路和采样修正
原因，不作为 v2 性能 baseline。该权限不应导致 workflow 或日志扩大密钥、Provider 或 benchmark
敏感数据暴露。
