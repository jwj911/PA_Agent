...
`RouteStep`，Task 8 已交付真实 `Stage2Step`，opt-in pipeline sequence 为
`Stage1Step -> RouteStep -> Stage2Step -> legacy_persist`；后者只承接已组装的 full/partial
写入边界，真实 `PersistStep` 尚未实现。`TwoStageOrchestrator.submit()` 默认行为保持不变。
Stage 1 的 happy/retry/network/validation/cancel/incremental、RouteStep 的 callable/object
router/策略顺序/经验限制/空经验库/`current_bars`/Stage 2 前取消/`route_failed`，以及 Stage2Step
的 continuation/flags/gate/流式/retry/network/validation/cancel/partial record 等价均已有测试
证据；Persist 独立步骤和完整尾部步骤化仍待完成。
...
| L3 | 引入 Pipeline Builder（Stage1Step/RouteStep/Stage2Step 已交付） | `orchestrator/pipeline/`、`orchestrator/two_stage.py`、`tests/unit/test_pipeline.py`、`tests/integration/test_stage1_pipeline_step.py`、`tests/integration/test_route_pipeline_step.py`、`tests/integration/test_stage2_pipeline_step.py` | state 已承载阶段 payload、usage、route 输出、Stage 2 flags、持久化意图和安全摘要；opt-in sequence 已为 `Stage1Step -> RouteStep -> Stage2Step -> legacy_persist`；`legacy_persist` 只承接已组装的 full/partial 写入边界，真实 `PersistStep` 仍需实现并验证旧/新事件与记录等价，默认 `submit()` 保持兼容 |
...
1. **继续推进 L3 等价 harness**：在已交付 `Stage1Step`、`RouteStep` 和 `Stage2Step` 的基础上
   实现真实 `PersistStep`，结合现有测试与真实 Provider runner、JSONL 事件重放验证 Pipeline
