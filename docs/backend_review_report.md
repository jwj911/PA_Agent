...
- L4 的热路径代码优化已完成；本轮新增 `pa-agent.performance.v1` 固定 synthetic benchmark、
  ...
  留存；固定 runner baseline 和同环境 regression 比较仍待完成。
...
4. **完成 L4 持续回归接线与 L1 扩展治理**：L4 固定 fixture 的 p50/p95 runner、手动/夜间
   budget workflow 和 artifact 已建立，下一步维护固定 runner baseline 并启用同环境回退告警；
   L1 已完成固定样例的 versioned/legacy 扩展兼容观察，下一步只观察真实已安装扩展和长期
   版本策略；不凭感觉修改热路径。
