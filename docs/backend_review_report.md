...
- L1 数据源/Provider 注册表已完成第二阶段基础，未知数据源配置已安全回退并持久化规范化值；
  ...
  测试；本轮补齐已安装 entry point 发现、registrar 契约和扩展失败隔离，进入外部扩展兼容观察期。
...
| 编号 | 任务 | 目标文件 | 预期收益 |
|---|---|---|---|
| L1 | Provider/数据源注册表与扩展契约（治理切片完成，进入观察期） | `pa_agent/extensions.py`、`ai/client_factory.py`、`ai/provider_registry.py`、`data/factory.py`、`data/registry.py`、`tests/unit/test_extensions.py`、`tests/unit/test_registry.py` | AI client 与数据源支持规格注册、优先级/延迟 builder、运行时扩展、entry point registrar、失败隔离和生命周期/并发/lazy-import 证据；外部扩展兼容观察和版本化契约策略仍待完成，Provider 同步仍由现有 service 负责 |
...
4. **完成 L4 持续回归接线与 L1 扩展治理**：L4 固定 fixture 的 p50/p95 runner 和当前报告
   已建立，下一步接入 CI/夜间任务并维护同环境 baseline；不凭感觉修改热路径。
