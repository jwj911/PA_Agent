...
> 状态更新：2026-07-22。R1-R8、M1-M10 及 L1-L6 的实际进度以
...
- L5 已接入 K 线几何相似度；本轮新增版本化脱敏评估合同、离线
  ...
  `pa-agent.experience-split.v1` 固定切分/dataset digest，但经验目录仍为空，不能据此判断
  真实交易结构检索质量，也不应调整线上权重。
- L4 的热路径代码优化已完成；本轮新增 `pa-agent.performance.v1` 固定 synthetic benchmark、
  p50/p95 budget 和 10% regression 判定，生成当前 Windows/Python 报告；本轮又接入
  `.github/workflows/l4-benchmark.yml` 的 Windows/Python 3.12.9 手动/夜间预算门禁和 artifact
  留存，并按 iterations/warmups 缓存最近一次成功 baseline。首次 `workflow_dispatch` 已由
  run `29923921295` 成功完成并上传 `l4-benchmark-29923921295`；第二次同参数对照和 runner
  image 变更审核仍待完成。
- L6 已完成 `AppEvent`/`EventSink`、`bootstrap_headless()`、共享 core/GUI bootstrap 边界，
  ...
  `tools/validate_live_observation.py` 离线核对 summary/event/record 自洽性，但不替代真实
  Provider 或 GUI/headless 等价证据。
...
1. **完成 L6 真实环境收口**：在 fixture 等价、`pa-agent.event.v1` envelope、严格跨进程
   replay contract、显式 live harness 和 artifact validator 基础上，继续保持真实 Provider
   仅显式执行，收集真实运行 record/事件和 record/event 完整等价证据。
2. **推进 L3 rollout evidence**：保持 `orchestrator.pipeline_builder_enabled=false` 为默认，
   使用显式 live harness 分别运行 legacy 和 flag-on，结合真实 Provider runner 验证 Pipeline
   state/step 与旧 `submit()` 的 record/事件等价，满足后才评估启用默认 flag。
3. **等待数据后推进 L5 离线评估**：固定切分合同已建立，下一步导出真实脱敏案例和人工标签，
   生成 Recall/NDCG、fallback 和 ranking stability 报告。
4. **完成 L4 持续回归接线与 L1 扩展治理**：L4 固定 fixture 的 p50/p95 runner、手动/夜间
   budget workflow、最近成功 baseline cache 和 artifact 已建立，首轮 Actions baseline 已由
   run `29923921295` 产生；下一步用相同参数完成 `--baseline` p95 对照并审核 runner image
   变化；L1 已完成固定样例的 versioned/legacy 扩展兼容观察，
...
L4 hosted baseline 的手动执行前提已固定：网页入口要求仓库 Write 权限；CLI/API 入口使用
仅限 `PA_Agent` 的 Fine-grained PAT，Repository `Actions: Read and write`。首次运行
`iterations=30`、`warmups=5` 建立 baseline，第二次同参数运行验证 cache restore、p95
regression 和 JSON artifact；首轮 run `29923921295` 已证明 baseline save/upload 链路可用，
但尚未证明第二次 `--baseline` 对照。该权限仅用于触发/观察 workflow，不应导致 workflow 或日志扩大
密钥、Provider 或 benchmark 敏感数据暴露。
