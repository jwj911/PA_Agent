...
- L4 的热路径代码优化已完成；本轮新增 `pa-agent.performance.v1` 固定 synthetic benchmark、
  ...
  留存，并按 iterations/warmups 缓存最近一次成功 baseline；首次 Actions 运行和 runner image
  变更审核仍待完成。
...
4. **完成 L4 持续回归接线与 L1 扩展治理**：L4 固定 fixture 的 p50/p95 runner、手动/夜间
   budget workflow、最近成功 baseline cache 和 artifact 已建立，下一步由 Actions 产生首轮
   baseline 并审核 runner image 变化；L1 已完成固定样例的 versioned/legacy 扩展兼容观察，
...
regression 和 JSON artifact。该权限仅用于触发/观察 workflow，不应导致 workflow 或日志扩大
