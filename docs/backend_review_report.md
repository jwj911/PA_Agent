...
- L6 已完成 `AppEvent`/`EventSink`、`bootstrap_headless()`、共享 core/GUI bootstrap 边界，
  ...
  并固定 `pa-agent.event.v1` envelope。真实 Provider 环境观察、真实运行 record/事件证据和
  跨进程 correlation 重放仍未收敛。
...
1. **完成 L6 真实环境收口**：在本轮 fixture 等价证据和 `pa-agent.event.v1` envelope 基础上，
   继续保持真实 Provider 仅显式执行，收集真实运行 record/事件和跨进程 correlation 重放证据。
