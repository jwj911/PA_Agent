...
| L3 Pipeline Builder | Stage 1/Route/Stage 2 真实步骤已交付，默认旧路径 | 新增 PyQt-free `orchestrator/pipeline/`、`PipelineState`、`TerminalStatus`、`PersistenceIntent`、`PipelineStep`、`StepResult`、`PipelineBuilder`、`Stage1Step`、`RouteStep` 和 `Stage2Step`；opt-in `run_pipeline()` 顺序为 `Stage1Step -> RouteStep -> Stage2Step -> legacy_persist`；state 显式承载阶段 payload、usage、route 输出、Stage 2 flags、partial reason 和持久化意图；Stage2Step 保持 continuation、gate、流式、retry、network、validation、cancel 语义；`legacy_persist` 只承接已组装的 full/partial 写入边界，真实 `PersistStep` 尚未实现 | `PersistStep` 真实步骤、全终态与 GUI/headless 等价验证、feature flag 观察周期；默认路径仍为旧 `submit()` |
...
Task 8 已交付 `Stage2Step` 和 opt-in 等价证据；Persist 仍由 `legacy_persist` 承接已组装的
full/partial 写入边界。L5/L4 在真实数据或固定 benchmark 建立前，不得
宣称收敛或直接调整线上权重/热路径。
...
Task 8 已将 Stage 2 准备/执行拆为真实 `Stage2Step`；其后仍由 `legacy_persist` 承接已经
组装完成的 full/partial 写入边界，默认 `submit()` 路径保持不变。
...
仍留待后续切片。`Stage1Step`、`RouteStep` 和 `Stage2Step` 已是真实步骤，其中 RouteStep
负责复用 router/经验读取并写入 route state，Stage2Step 负责 continuation、flags、gate、
Provider 调用、流式、retry、失败和取消边界；`legacy_persist` 只承接 Stage 2/既有 helper
组装后的 full/partial 写入边界，尚不等同于真实 `PersistStep`。
...
6. **已完成 Task 8**：把 Stage 2 continuation 构建、settings flags、gate short-circuit、
   ...
   opt-in sequence 固定为 `Stage1Step -> RouteStep -> Stage2Step -> legacy_persist`，并通过
   `persist=False` 把 record 组装与写入边界分开。
7. **仍待 Task 9**：把 final/partial record 组装、partial reason、脱敏、磁盘错误和
   `RecordSaved` 契约集中到真实 `PersistStep`；当前 `legacy_persist` 不得被称为 PersistStep。
8. 保留 `submit()` 作为 compatibility facade，默认调用旧路径。
9. 开启 Pipeline feature flag，观察至少一个完整发布周期。
10. 删除旧 submit 内部实现，只保留 facade。
...
- `Stage1Step` 的 happy、retry、network、validation、cancel、incremental 路径以及最终 record/
  ...
  record 等价测试；Persist 的独立步骤和完整尾部等价仍待完成；
- route failure 已产生稳定的 `route_failed`；Stage2 failure 已有显式终态；persist failure 仍
  需在真实 `PersistStep` 中收口；
  且摘要不会暴露
...
- 已完成 Task 6 的 `Stage1Step`、Task 7 的 `RouteStep` 和 Task 8 的 `Stage2Step` 真实步骤；
  opt-in sequence 为 `Stage1Step -> RouteStep -> Stage2Step -> legacy_persist`，其中
  `legacy_persist` 只承接已组装的 full/partial 写入边界；
- 下一步将 `legacy_persist` 的职责逐项转成真实 `PersistStep`；
...
`tests/integration/test_route_pipeline_step.py` 和 Stage 2 集成测试
`tests/integration/test_stage2_pipeline_step.py` 同时纳入 CI targeted pytest 与 focused
Ruff/Black 目标清单；PersistStep 的独立测试待后续切片补齐。
...
Task 6 完成 `Stage1Step`，Task 7 完成 `RouteStep`，Task 8 完成 `Stage2Step`；当前主线继续
转入真实 `PersistStep`、最终 record 等价和事件重放；L2 只保留兼容观察期。
