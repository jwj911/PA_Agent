...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1、M2.1、M2.2 已完成；旧 loader/fallback 按政策 retain | TemplateStore、TemplateContext、29 个稳定 Prompt ID 与 manifest 映射、ID 路由/legacy 投影、shared system/Stage 1 ID loader、Stage 2/continuation golden snapshot 和回退观察已完成 | 执行 M2.3 Stage 2 与 TemplateContext 内部 ID 切换；最早 0.3.0 且具备 v0.2.0 tag、fallback 零命中和 golden 报告后才评估删除旧入口 |
...
M1 已新增 29 个稳定 ID、严格 Catalog、ID 依赖关系和 TemplateStore 显式 ID API；M2.1
已把 router 主逻辑切换为 ID，并由 Catalog 生成 legacy 文件名兼容投影；M2.2 已将 shared
system 与 Stage 1 builder 切换到原子 ID loader。旧 `TemplateSpec.name`、文件名路由/列表 API、
Prompt 正文、JSON schema 和 golden 保持不变。后续从 M2.3 Stage 2/TemplateContext 继续，
仍须按文档中的原子切片逐层推进，不得与多 Agent、Prompt 文本优化或经验库权重调整混在同一
提交。
