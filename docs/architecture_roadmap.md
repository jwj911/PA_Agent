...
已完成，不再阻塞 L6；后续只观察外部扩展兼容性。JSONL 事件端口已建立，但 L6 仍需真实 Provider runner
...
- 公开 `HeadlessAnalysisAdapter` 已交付；下一步补 GUI/headless 最终/partial/cancel/failure
  record 等价和事件 envelope 版本契约。
