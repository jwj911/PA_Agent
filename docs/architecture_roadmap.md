...
| L3 Pipeline Builder | Stage 1 真实步骤已交付，默认旧路径 | 新增 PyQt-free `orchestrator/pipeline/`、`PipelineState`、`TerminalStatus`、`PersistenceIntent`、`PipelineStep`、`StepResult`、`PipelineBuilder` 和 `Stage1Step`；opt-in `run_pipeline()` 顺序为 `Stage1Step -> legacy_post_stage1`；state 显式承载阶段 payload、usage、route 输出、partial reason 和持久化意图；`legacy_post_stage1` 继续承接 Route/Stage 2/Persist 兼容尾步骤 | Route/Stage 2/Persist 真实步骤、全终态与 GUI/headless 等价验证、feature flag 观察周期；默认路径仍为旧 `submit()` |
...
协议并完成 Task 5 的 state foundation；Task 6 已交付 `Stage1Step` 真实步骤和 opt-in 等价证据，
但 Route/Stage 2/Persist 仍由 `legacy_post_stage1` 兼容尾步骤承接。L5/L4 在真实数据或固定 benchmark 建立前，不得
宣称收敛或直接调整线上权重/热路径。
...
Task 6 已将 Stage 1 拆为真实 `Stage1Step`，其后仍由 `legacy_post_stage1` 作为 Route/Stage 2/Persist
兼容尾步骤承接，默认 `submit()` 路径保持不变。
...
仍留待后续切片。`Stage1Step` 已是真实 Stage 1 步骤，但 Route/Stage 2/Persist 仍通过
`legacy_post_stage1` 兼容尾步骤执行。Task 5 的安全摘要只提供 allowlist metadata、阶段形状、
route 数量、usage 计数和稳定终态，不等同于真实持久化步骤。
...
4. **已完成 Task 6**：把现有 Stage 1 helper 适配为真实 `Stage1Step`，并在 opt-in pipeline
   中按 `Stage1Step -> legacy_post_stage1` 执行；Stage 1 集成测试覆盖 happy/retry/network/
   validation/cancel/incremental，且验证最终 record 与事件等价。
5. 把 `_route_and_load_experience`、`_try_gate_short_circuit`、`_run_stage2`、`_persist_result`
   逐步适配为真实步骤；当前 Route、Stage 2、Persist 仍由 `legacy_post_stage1` 兼容尾步骤承接。
6. 保留 `submit()` 作为 compatibility facade，默认调用旧路径。
7. 开启 Pipeline feature flag，观察至少一个完整发布周期。
8. 删除旧 submit 内部实现，只保留 facade。
...
- `Stage1Step` 的 happy、retry、network、validation、cancel、incremental 路径以及最终 record/
  event equivalence 已有 opt-in 测试证据；Route、Stage 2、Persist 的对应真实步骤和完整尾部
  等价仍待完成；
- route/persist failure 分别产生稳定的 `route_failed`/`persist_failed`，且摘要不会暴露
...
- 已完成 Task 6 的 `Stage1Step` 真实步骤；opt-in sequence 为
  `Stage1Step -> legacy_post_stage1`，其中 Route/Stage 2/Persist 仍是兼容尾步骤；
- 下一步将兼容尾步骤中的 Route/Stage 2/Persist 逐项转成真实 state/step；
...
Stage 1 集成测试 `tests/integration/test_stage1_pipeline_step.py` 同时纳入 CI targeted
pytest 和 focused Ruff 目标清单；Route/Stage 2/Persist 的真实步骤测试待后续切片补齐。
...
第 238 轮已完成 L3 state/step compatibility adapter，本轮完成 Task 5 PipelineState foundation；
当前主线继续转入 L3 真实步骤、最终 record 等价和事件重放；L2 只保留兼容观察期。
