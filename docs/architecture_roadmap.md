...
| L6 无 GUI 运行 | Core/GUI bootstrap 边界拆分完成 | `AppEvent`、`EventSink`、`CollectingEventSink` 已可无 Qt import 使用；`AppContext._build_core()` 由 `bootstrap_gui()`/`bootstrap_headless()` 复用，`bootstrap()` 委托 GUI 路径 | 增加 CLI 入口、GUI/headless 同 snapshot 等价测试，并继续收口公开 adapter 契约 |
...
`EventBus` import、无数据源连接。CLI runner 和 GUI/headless 同 snapshot 等价测试仍未建立。
...
- 下一步用 headless runner 建立跨层测试 harness。
...
下一轮建议从 **L6 headless runner / CLI 最小入口与 GUI/headless 同 snapshot 等价测试**开始，
而不是立即调整 L5 相似度权重。
