...
| L3 Pipeline Builder | PipelineState foundation 已交付，默认旧路径 | 新增 PyQt-free `orchestrator/pipeline/`、`PipelineState`、`TerminalStatus`、`PersistenceIntent`、`PipelineStep`、`StepResult`、`PipelineBuilder`；state 显式承载 Stage 1/Stage 2 payload、usage、route 输出、partial reason 和持久化意图，补充 route/persist 终态映射与安全摘要/URL 脱敏；`run_pipeline()`/`submit_pipeline()` 仍提供 opt-in legacy adapter | Stage 1/route/Stage 2/persist 真实步骤、全终态与 GUI/headless 等价验证、feature flag 观察周期；默认路径仍为旧 `submit()` |
...
协议并完成 Task 5 的 state foundation，但真实 Stage 1/route/Stage 2/persist 步骤和完整等价
实现仍未完成。L5/L4 在真实数据或固定 benchmark 建立前，不得
宣称收敛或直接调整线上权重/热路径。
...
wrapper 目前仍把完整 `submit()` 作为单个兼容步骤。
...
与 Stage 1/route/Stage 2/persist 的细粒度步骤留待后续切片。Task 5 的安全摘要只提供
allowlist metadata、阶段形状、route 数量、usage 计数和稳定终态，不等同于真实持久化步骤。
...
4. 把现有 `_build_empty_record` 和 usage helper 包装为 state factory。
5. 把已拆出的 `_run_stage1`、`_route_and_load_experience`、`_try_gate_short_circuit`、
   `_run_stage2`、`_persist_result` 分别适配为真实步骤；当前这些真实 `PipelineStep` 尚未实现。
6. 保留 `submit()` 作为 compatibility facade，默认调用旧路径。
7. 新旧路径执行相同 fixture，比较事件序列、partial record 和最终 record。
...
- 下一步将现有方法拆分结果转成真实 state/step，当前 Stage 1/Route/Stage 2/Persist 仍未实现；
