...
| L6 无 GUI 运行 | 事件端口第一阶段完成 | `AppEvent`、`EventSink`、`CollectingEventSink` 已可无 Qt import 使用；`EventBus.publish()` 可把应用事件转发到旧 Qt signals | 拆分 headless/gui bootstrap，增加 CLI 入口和同 snapshot 等价测试 |
...
`EventBus.publish()` 能把应用事件转发到旧 Qt signals；但 `AppContext.bootstrap()` 尚未拆分，
GUI 与 headless 的装配仍未分离。
