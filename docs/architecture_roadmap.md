...
| L3 Pipeline Builder | Stage 1/Route 真实步骤已交付，默认旧路径 | 新增 PyQt-free `orchestrator/pipeline/`、`PipelineState`、`TerminalStatus`、`PersistenceIntent`、`PipelineStep`、`StepResult`、`PipelineBuilder`、`Stage1Step` 和 `RouteStep`；opt-in `run_pipeline()` 顺序为 `Stage1Step -> RouteStep -> legacy_stage2_persist`；state 显式承载阶段 payload、usage、route 输出、partial reason 和持久化意图；RouteStep 保持 callable/object router、策略顺序、经验限制、`current_bars`、空经验库和 Stage 2 前取消边界，并映射 `route_failed`；`legacy_stage2_persist` 继续承接 Stage 2/Persist | Stage 2/Persist 真实步骤、全终态与 GUI/headless 等价验证、feature flag 观察周期；默认路径仍为旧 `submit()` |
...
其中 L1 已具备第二阶段注册表基础，未知数据源配置回退和第一批生命周期/并发证据已完成，不再阻塞 L6；
下一步只剩插件发现方案与正式扩展契约。JSONL 事件端口已建立，但 L6 仍需真实 Provider runner
环境验证、record 等价证据和公开 adapter 契约。L3 已建立 `orchestrator/pipeline/` 状态/步骤
协议并完成 Task 5 的 state foundation；Task 6 已交付 `Stage1Step`，Task 7 已交付 `RouteStep`
和 opt-in 等价证据，但 Stage 2/Persist 仍由 `legacy_stage2_persist` 兼容尾步骤承接。L5/L4 在真实数据或固定 benchmark 建立前，不得
宣称收敛或直接调整线上权重/热路径。
...
其后仍由 `legacy_stage2_persist` 作为 Stage 2/Persist 兼容尾步骤承接，默认 `submit()` 路径保持不变。
...
仍留待后续切片。`Stage1Step` 和 `RouteStep` 已是真实步骤，其中 RouteStep 负责复用 router、
经验读取并写入 route state；Stage 2/Persist 仍通过 `legacy_stage2_persist` 兼容尾步骤执行。
Task 5 的安全摘要只提供 allowlist metadata、阶段形状、route 数量、usage 计数和稳定终态，
不等同于真实持久化步骤。
...
- `partial_reason`、`PersistenceIntent`、record、events、step history 和 terminal status；
...
5. **已完成 Task 7**：把 `_route_and_load_experience` 适配为真实 `RouteStep`，并在 opt-in
   pipeline 中按 `Stage1Step -> RouteStep -> legacy_stage2_persist` 执行；保持 callable/object
   router、策略文件顺序、经验数量/字符限制、`current_bars`、空经验库和 Stage 2 前取消边界，
   route exception 映射为 `route_failed` partial terminal。
6. 把 `_try_gate_short_circuit`、`_run_stage2`、`_persist_result` 逐步适配为真实步骤；当前
   Stage 2、Persist 仍由 `legacy_stage2_persist` 兼容尾步骤承接。
7. 保留 `submit()` 作为 compatibility facade，默认调用旧路径。
8. 开启 Pipeline feature flag，观察至少一个完整发布周期。
9. 删除旧 submit 内部实现，只保留 facade。
...
- `Stage1Step` 的 happy、retry、network、validation、cancel、incremental 路径以及最终 record/
  ...
  experience limit、empty library、`current_bars`、取消和 `route_failed` 测试；Stage 2、Persist
  的对应真实步骤和完整尾部等价仍待完成；
- route failure 已产生稳定的 `route_failed`；persist failure 仍需在真实 `PersistStep` 中收口；
  且摘要不会暴露
...
GUI/headless 全链路等价测试、真实 Provider 环境验证和公开 adapter 契约仍未建立。
...
- 已新增显式 `--run/--execute` 两阶段 runner、final/partial record 持久化、退出码映射和
  correlation JSONL 事件；
- 下一步补 GUI/headless 最终/partial/cancel/failure record 等价和公开 adapter 契约。
...
- 已完成 Task 6 的 `Stage1Step` 和 Task 7 的 `RouteStep` 真实步骤；opt-in sequence 为
  `Stage1Step -> RouteStep -> legacy_stage2_persist`，其中 Stage 2/Persist 仍是兼容尾步骤；
- 下一步将兼容尾步骤中的 Stage 2/Persist 逐项转成真实 state/step；
...
- Pipeline：每个 terminal status、`Stage1Step` 的 happy/retry/network/validation/cancel/incremental
  ...
  取消和 `route_failed`，以及事件序列和最终 record equivalence；
...
Stage 1 集成测试 `tests/integration/test_stage1_pipeline_step.py` 和 Route 集成测试
`tests/integration/test_route_pipeline_step.py` 同时纳入 CI targeted pytest 与 focused
Ruff/Black 目标清单；Stage 2/Persist 的真实步骤测试待后续切片补齐。
...
Task 6 完成 `Stage1Step`，Task 7 完成 `RouteStep`；当前主线继续转入 Stage 2/Persist 真实步骤、
最终 record 等价和事件重放；L2 只保留兼容观察期。
