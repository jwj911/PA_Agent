...
| L3 Pipeline Builder | state/step 第一切片已交付，默认旧路径 | 新增 PyQt-free `orchestrator/pipeline/`、`PipelineState`、`TerminalStatus`、`PipelineStep`、`StepResult`、`PipelineBuilder`；`TwoStageOrchestrator.run_pipeline()`/`submit_pipeline()` 提供 opt-in legacy adapter | 将 legacy wrapper 拆为 Stage 1/route/Stage 2/persist steps，并完成全终态与 GUI/headless 等价验证 |
...
协议，但真实阶段步骤和完整等价实现仍未完成。L5/L4 在真实数据或固定 benchmark 建立前，不得
宣称收敛或直接调整线上权重/热路径。
...
但 wrapper 目前仍把完整 `submit()` 作为单个兼容步骤。
...
当前第一切片实际包含 `state.py`、`step.py`、`steps.py`、`builder.py` 和包导出；`events.py`
与 Stage 1/route/Stage 2/persist 的细粒度步骤留待后续切片。
...
2. 把现有 `_build_empty_record` 和 usage helper 包装为 state factory。
3. 把已拆出的 `_run_stage1`、`_route_and_load_experience`、`_try_gate_short_circuit`、
   `_run_stage2`、`_persist_result` 分别适配为步骤。
4. 保留 `submit()` 作为 compatibility facade，默认调用旧路径。
5. 新旧路径执行相同 fixture，比较事件序列、partial record 和最终 record。
6. 开启 Pipeline feature flag，观察至少一个完整发布周期。
7. 删除旧 submit 内部实现，只保留 facade。
...
- 已建立显式 `PipelineState`/`TerminalStatus`/`PipelineStep`/`StepResult` 协议和 legacy wrapper；
- 下一步将现有方法拆分结果转成真实 state/step；
...
当前主线转入 L6 真实 runner、最终 record 等价和事件重放；L2 只保留兼容观察期。
