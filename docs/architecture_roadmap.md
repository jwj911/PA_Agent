...
| L6 无 GUI 运行 | Headless bootstrap 第一阶段完成 | `AppEvent`、`EventSink`、`CollectingEventSink` 已可无 Qt import 使用；`AppContext.bootstrap_headless()` 可无 Qt `EventBus` 装配核心组件 | 继续拆分 GUI adapters，增加 CLI 入口和同 snapshot 等价测试 |
...
`EventBus` 直接继承 `QObject`，`AppContext.bootstrap()` 创建 Qt EventBus 和所有 GUI 依赖。
第 223 轮已新增 PyQt-free `AppEvent` / `EventSink` / `CollectingEventSink`，并让
`EventBus.publish()` 能把应用事件转发到旧 Qt signals。第 224 轮新增
`AppContext.bootstrap_headless()`，可在不创建 Qt `EventBus`、不连接数据源的情况下装配核心组件；
但 GUI adapters 尚未从 `bootstrap()` 中独立出来，CLI runner 也尚未建立。
...
将 `AppContext.bootstrap()` 进一步拆成：

```text
AppContext.load_settings()
AppContext.build_core(...)
AppContext.build_gui_adapters(...)
```

当前已公开 `bootstrap_headless()`；后续建议继续公开/收口：

- `bootstrap_gui()`
- `build_core(...)`
- `build_gui_adapters(...)`

两者共享 settings、registry、client、data source、assembler、validator、records 和
pipeline services；只有 GUI 入口装配 Qt EventBus、MainWindow 和 Qt widgets。
...
pa-agent headless snapshot --symbol ... --timeframe ...
pa-agent headless analyze --input snapshot.json --output result.json
```
...
- 先抽象应用事件和 `EventSink`；
- 拆分 `AppContext` core/gui bootstrap；
- 用 headless runner 建立跨层测试 harness。
...
下一轮建议从 **L6 Headless core 的事件端口抽象**开始，而不是立即调整 L5 相似度权重。
