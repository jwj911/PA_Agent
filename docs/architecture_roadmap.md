...
| L6 无 GUI 运行 | headless 第一切片与公开 adapter 已交付，等价契约未收口 | `AppEvent`、`EventSink`、`CollectingEventSink`、`JsonlEventSink`、`replay_jsonl`、共享 `_build_core()`、`bootstrap_gui()`/`bootstrap_headless()`、兼容 `bootstrap()`、`HeadlessAnalysisAdapter` 和 PyQt-free `pa-agent headless`；显式 `analyze --run/--execute` 已接入两阶段 orchestrator、record 和 JSONL 事件 | GUI/headless 最终/partial/cancel/failure record 等价、事件 envelope 版本契约和真实 Provider 环境验证 |
...
环境验证、record 等价证据和事件 envelope 版本契约。L3 已建立 `orchestrator/pipeline/` 状态/步骤
...
持久化失败。Pipeline feature flag、稳定观察周期和 GUI/headless 全链路等价仍未收口。
...
本轮新增公开 `HeadlessAnalysisAdapter`，GUI/headless 全链路等价测试、真实 Provider 环境验证和
事件 envelope 版本契约仍未建立。
