...
| L6 无 GUI 运行 | 未启动 | `AppContext`、`EventBus`、编排层已经存在，但 `EventBus` 直接依赖 Qt | 抽象事件端口，增加 headless bootstrap 和 CLI 入口 |
...
这使得 headless 入口无法保证无 Qt import；同时编排层的进度通知和 GUI signal 形态混在一起。
...
兼容期间保留现有 `EventBus` 名称，由它实现 `EventSink` 并继续暴露 Qt signals。
headless 使用 `CollectingEventSink` 或 JSONL sink，不创建 `QApplication`。
