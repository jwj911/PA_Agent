...
| L6 无 GUI 运行 | CLI/JSONL 事件切片完成，runner 未收口 | `AppEvent`、`EventSink`、`CollectingEventSink`、`JsonlEventSink`、`replay_jsonl`、共享 `_build_core()`、`bootstrap_gui()`/`bootstrap_headless()`、兼容 `bootstrap()`，以及 PyQt-free `pa-agent headless` 已可用 | 真实 Provider runner、最终/partial record 等价测试和公开 adapter 契约 |
...
和 record 等价证据。L5/L4 在真实数据或固定 benchmark 建立前，不得宣称收敛
...
Stage 1 prompt 等价测试，第 234 轮已新增 PyQt-free JSONL event sink/replay；真实两阶段
Provider runner、最终 record 等价测试和公开 adapter 契约仍未建立。
...
- 下一步补真实 Provider runner、最终 record 等价和公开 adapter 契约。
