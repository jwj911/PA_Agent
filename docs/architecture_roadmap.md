...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1 Catalog 已完成；旧 loader/fallback 按政策 retain | TemplateStore、TemplateContext、29 个稳定 Prompt ID 与 manifest 映射、system/Stage 1/Stage 2/continuation golden snapshot 和回退观察已完成 | 执行 M2 路由与组装内部 ID 切换；最早 0.3.0 且具备 v0.2.0 tag、fallback 零命中和 golden 报告后才评估删除旧入口 |
...
M1 已新增 29 个稳定 ID、严格 Catalog、ID 依赖关系和 TemplateStore 显式 ID API；旧
`TemplateSpec.name`、文件名加载接口、路由、Prompt 正文、JSON schema 和 golden 保持不变。
后续从 M2 继续，仍须按文档中的原子切片逐层推进，不得与多 Agent、Prompt 文本优化或经验库
权重调整混在同一提交。
