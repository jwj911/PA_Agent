...
> 最近验收：GitHub Actions run `30507318293`，`main@47d0b76` 的
...
三轮真实稳定观察和默认切换。M4.3 已取得真实 Provider 通过证据；当前尚未满足外部证据
条件的路线为：
...
record、事件、prompt、流式内容、策略文件和写入边界对照，并连续完成 3 个独立真实成功
pair。6 个单体 validator 和 3 个 pair comparator 均为 `valid=true`，18 个本地产物文件的
明文密钥扫描为 0 命中。`pipeline_builder_enabled` 现默认为 `true`，旧配置缺少字段时也采用
新默认；显式 `false` 保留 legacy 回滚。L6 的 mock/fixed-fixture 全终态等价和真实成功主路径
证据均已完成。
...
默认值在迁移完成前保持关闭或兼容模式；三轮真实稳定观察通过后，
`orchestrator.pipeline_builder_enabled` 的默认值已切换为 `true`，旧配置缺少
`orchestrator` section/字段时同样采用 Pipeline。显式 `false` 保留 legacy 回滚；开关必须记录
在诊断日志中，便于比较新旧路径。
...
### 5.6 Prompt ID 与文件名解耦

解耦前，29 个运行时模板以中文 `.txt` 文件名同时承担 manifest 主键、路由结果、模型输出、
Pipeline 状态、记录字段和 GUI 展示值。为避免后续重命名或 `.prompt.md` 迁移形成跨层破坏，
项目按 [`prompt_id_decoupling_plan.md`](./prompt_id_decoupling_plan.md) 分阶段迁移：

- 以稳定 `PromptId` 作为核心身份，以可变 `source_path` 定位文件；
- 保留不可变 `legacy_filename` 兼容旧 API 和旧记录；
- M1 已完成 catalog、manifest 和 store ID 双入口；M2-M3 再迁移 router、assembler 和
  Pipeline/record，Prompt 字节必须不变；
- M4 单独移除模型输出文件名的职责并完成合同评估；
- M5 才可选执行 `.txt` 到 `.prompt.md` 的物理存储迁移。
...
访问，ID/file 不一致时失败关闭。M3.2 已让 `PipelineState`、RouteStep、Stage 2 snapshot 和
PersistStep 双写 `strategy_prompt_ids` 与 legacy filename route output；旧 router、Stage 2
filename 参数和 callback 保持兼容，安全摘要只记录数量。旧 `TemplateSpec.name`、文件名路由/
列表 API、Prompt 正文、JSON schema 和 golden 保持不变。M3.3 已让 Prompt 调试面板显示
`display_name [prompt_id]`，相对路径只进入 tooltip；旧 filename GUI 方法与 callback 继续兼容，
未知旧值仅标记 unresolved。M1-M3 已完成且保持 Prompt 字节不变。后续 M4 单独清理模型输出
filename 的职责并执行合同评估，M5 再决定物理后缀迁移；不得与多 Agent、Prompt 文本优化或
经验库权重调整混在同一提交。

M4.1 已升级 Stage 1 schema 为 `pa-agent.stage1-output.v2`，模型输出不再要求
`strategy_files_needed`；Normalizer 仍接受旧字段/alias，但兼容字段始终由 router 重新投影，
...
减少 7，单项最大上浮约 0.0089%。历史 3 组真实 pair 又建立了 6 条 M3-compatible 基线：
0 次终局校验失败、0 次验证重试，模型 filename 输出和 router 冲突均为 6/6；fixture/provider
合同哈希各自唯一。2026-07-29 的 M4 候选 legacy/Pipeline 使用相同 Provider/fixture 合同，
两条路径均完成 5 事件、调用 Provider 并写入 record；终局校验失败和重试均为 0/2，模型
filename 输出与 router 冲突均由基线 6/6 降至 0/2，平均输入 token 下降约 0.769%。
comparison 的 11 项 gate 全部通过，最终机器报告为 `m4_exit_gate_passed=true`。M3.3 token
基线固定使用 `tiktoken 0.12.0`；项目依赖与基线版本由单测强制一致，升级 tokenizer 时必须
重建 M3 基线和 M4 报告。M4 已收口。M5.0 诊断 `PA-M5-SOURCE-PATH-001` 又确认 assembler
fallback 与默认 decision tree 仍直读 legacy `.txt` 物理路径；本轮先让二者经 Catalog/Prompt ID
读取并增加迁移态回归，该切片已通过本地与远端门禁。M5.1 随后只对无模板依赖的
`pa.persona` 执行首个 `git mv`，把 `source_path` 更新为 `.prompt.md`，同时保留稳定 ID、
manifest v2 和旧 `.txt` `legacy_filename`。磁盘原始字节、`TemplateStore` 正文、M4 Prompt
metrics 与 Stage 1/三种 Stage 2 组装摘要均与迁移前冻结值一致；实现提交 `fe119af` 和
GitHub Actions run `30474245087` 的双矩阵已通过，后续按同一证据标准迁移剩余 28 个模板。
M5.2 将高耦合 `pa.binary_decision` 单独迁移为 `.prompt.md`：除双口径正文与四种组装摘要
不变外，默认决策树仍返回旧 `.txt` `source`，14 个 sections、54 个节点和规范 JSON 摘要
均保持一致；实现提交 `980c294` 和 GitHub Actions run `30490054789` 的双矩阵已通过，
后续可按依赖边界启动其 5 个依赖模板批次。
M5.3 随后成组迁移 5 个直接依赖的 Stage 1/Base 模板；每个模板的 raw/TemplateStore
...
组装摘要和 M4 最终报告均与迁移前证据一致。实现提交 `47d0b76` 和 GitHub Actions run
`30507318293` 的双矩阵已通过，M5.4 已收口；之后继续迁移剩余 12 个独立 pattern/context
模板。
M5.5 已迁移最后 12 个 channel width、pattern 和 context 模板；迁移合同现覆盖 29/29，
运行时目录为 29 个 `.prompt.md`、0 个 `.txt`。正文、10 组路由/Stage 2 组装摘要和 M4
最终报告均与迁移前证据一致；本地全部门禁已通过，远端双矩阵和最终完成审计通过前不标记
M5 收口。
...
`PersistStep`；Task 10 已接入 rollout flag、完整终态矩阵和 Qt-free adapter equivalence 测试。
三轮真实 pair 通过后，四个步骤已成为默认 `submit()` 路径；legacy 由显式 flag-off 保留。
...
8. 保留 `submit()` 作为 compatibility facade；flag-on 默认委托完整四步 Pipeline，显式
   flag-off 调用旧路径。
9. **已完成 Task 10 rollout 准备**：新增
   `orchestrator.pipeline_builder_enabled`（该阶段默认 `false`），为 Settings round-trip 和
   缺少 `orchestrator` section 的旧配置补充当时的 legacy 默认，接入 flag 路由，建立完整终态矩阵及
   Qt-free headless/GUI adapter equivalence 测试，并将测试/config module 纳入 CI targets。
10. **已完成受控 fixture rollout 观察**：在切换默认 flag 前，以 5 个终态场景、
    每场景 3 轮比较 flag-off/flag-on 的 record、事件、prompt、流式内容、策略文件和写入边界；
    2026-07-23/24 连续 3 个真实 Provider pair 均通过；GUI/headless
    final/partial/cancel/failure evidence 继续由固定 fixture 矩阵覆盖。
11. 上述观察无未解释偏差后，默认 flag 已切换为 `true`。旧 `submit()` 内部实现继续作为
    显式 `false` 回滚路径保留，尚不评估删除。
...
- `Stage1Step` 的 happy、retry、network、validation、cancel、incremental 路径以及最终 record/
  ...
  `submit()` flag-off/flag-on 路由、Settings round-trip/缺失字段采用当前默认以及 Qt-free
...
- 四个真实步骤与 Pipeline feature flag 接线已齐；受控 fixture rollout 已覆盖 5 个终态场景
  × 3 轮，3 个连续真实 pair 均通过，GUI/headless final/partial/cancel/failure 固定 fixture
  全链路等价已完成；
- 任一阶段出现差异可按 symbol/timeframe 或配置开关回退。
- 默认切换后继续通过生命周期日志观察；出现未解释的 terminal、event 或 record 偏差时，
  显式设置 `orchestrator.pipeline_builder_enabled=false` 回滚，并保留 legacy facade。
...
`docs/experience_evaluation_runbook.md`。同一管道现提供
`pa-agent.experience-eval-readiness.v1` 脱敏预检，分别判定 annotation export 和 evaluation
是否 ready，并以稳定 blocker code 拒绝缺 salt、无案例、catalog 无效、instrument group
不足或 annotations 未就绪；输出只包含聚合计数，不暴露 symbol、价格、K 线、路径或 salt。
`pa_agent.records.experience_curation` 与
...
导入门禁。真实 CLI 导入还必须提供本地 evidence file；`pa-agent.outcome-evidence.v1` 只保存
`realized-net-pnl-sign.v1`、allowlist evidence type 和文件 SHA-256，不保存文件名、路径或
市场数据。离线 catalog loader 复用同一 validator，使缺失或篡改 evidence 的旧/API 案例不能
进入 annotation export、readiness 或 evaluate；在线读取和 fallback 行为不变。
...
- 已完成 Task 6 的 `Stage1Step`、Task 7 的 `RouteStep`、Task 8 的 `Stage2Step` 和 Task 9
  的 `PersistStep` 真实步骤；Task 10 已完成 flag 接线、完整终态矩阵和 Qt-free
...
- 继续用 headless harness 做新旧事件和记录等价性的持续观察；GUI/headless
  final/partial/cancel/failure 全链路 fixture evidence 已完成；
- 显式 `tools/run_live_headless_observation.py --pipeline-builder-enabled` 只对本次有授权运行
  打开 Pipeline，未传参数保持 legacy；
- 默认 flag 已在三轮真实 pair 和 GUI/headless 全终态 fixture evidence 无未解释偏差后切换为
  `true`；显式 `false` 继续回滚 legacy，旧路径暂不下线。
...
`pa_agent/config/orchestrator.py` 纳入 focused Ruff/config target。GUI/headless 全链路
fixture 等价已完成，真实成功路径由三轮 pair 覆盖。
