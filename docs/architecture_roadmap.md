...
> 最近验收：GitHub Actions run `30474245087`，`main@fe119af` 的
...
均保持一致；完整本地/远端门禁通过前不启动其 5 个依赖模板批次。
