...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1-M3、M4.1 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、Stage 1 schema v2 与 router 权威兼容 Normalizer 已交付 | 执行 M4.2 Prompt/golden 清理与 M4.3 独立评估，再按证据决定 M5 后缀迁移；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1-M3、M4.1 已完成；Stage 1 schema v2 不再要求 filename，Normalizer 兼容字段由 router 唯一投影 | 下一切片为 M4.2：从 Prompt/增量/carryover 合同移除 `strategy_files_needed` 与物理 filename 引用并更新 golden；随后执行 M4.3 评估 |

当前外部证据主链路只剩 **L5**；在等待真实 evidence 期间，可按
[`prompt_id_decoupling_plan.md`](./prompt_id_decoupling_plan.md) 推进不依赖外部数据的 L2
Prompt ID 解耦。L3/L6 已收口并进入持续观察，L1 的实现与下线策略按政策 retain，L4 已进入
每日观察。
...
1. **L2/M4.2：清理 Prompt/golden 的 filename 职责**。M4.1 已完成 schema v2 与 router
   权威 Normalizer；下一步从全量/增量 Prompt 和 Stage 2 carryover 中移除
   `strategy_files_needed` 与物理 filename 交叉引用，固定预期 Prompt 漂移并更新 golden。
...
进入 M5 前完成离线与真实 Provider 对照。M4.1 已完成 schema/Normalizer 边界；M4.2 负责
Prompt/golden，M4.3 负责离线指标与真实观察。
