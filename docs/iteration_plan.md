...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1 Catalog 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID 与 manifest 映射、全部阶段迁移、TemplateContext、golden snapshots 和整组回退已交付 | 执行 M2 路由与组装内部 ID 切换；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1 Catalog 已完成，router、assembler、Pipeline 和 record 仍以 `.txt` 文件名传递 | 下一切片为 M2 router 双入口；先建立 ID 路由与 filename 兼容投影，再逐层切换 assembler，全部 Prompt digest 必须不变 |
...
1. **L2/M2：路由与组装内部切换到 ID**。先增加 `route_strategy_prompt_ids()` 和旧文件名
   等价投影，再依次切换 shared system、Stage 1、Stage 2；不改变 Prompt 正文、JSON schema
   或 golden。
...
第 233 轮模板引擎迁移和 Prompt ID M1 Catalog 已完成；M2-M3 只做结构解耦并保持 Prompt
