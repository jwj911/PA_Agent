...
当前外部证据主链路为 **L6 → L3 → L5**；L1/L2 的实现与下线策略已收口并按政策 retain，
L4 已进入每日观察，不再是当前阻塞项。
...
3. **L5：真实案例 opaque 导出、人工标注和离线指标报告**。
...
收尾边界：当前环境没有 `PA_AGENT_LIVE_API_KEY`，仍需授权环境生成至少一个真实
`pa-agent.live-observation-pair-validation.v1` 且 `valid=true` 的 pair。
