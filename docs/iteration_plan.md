...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1-M4.2、M4.3a 离线评估已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、Stage 1 schema v2、router 权威兼容 Normalizer、manifest v2、无 filename 模型上下文和版本化离线报告已交付 | M4.3b 真实 Provider 观察等待会话级凭据；完成后再按证据决定 M5 后缀迁移；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| Blocked | L2 | M1-M4.2 与 M4.3a 离线评估已完成；固定合同中校验失败/重试不回退、路由冲突清零、token 通过门禁 | M4.3b 需要会话级 `PA_AGENT_LIVE_API_KEY` 执行真实 Provider 观察；当前不得进入 M5 |
...
1. **L2/M4.3b：执行真实 Provider 观察**。M4.3a 离线报告已证明固定合同的校验失败/重试
   不回退、Normalizer 后路由冲突由 2/4 降为 0/4，四种 Prompt 合计估算 token 减少 7。
   当前缺少会话级 `PA_AGENT_LIVE_API_KEY`，保持阻塞；不得读取持久化配置、伪造 live
   证据或提前进入 M5。
...
Prompt/carryover/golden 和 manifest v2，M4.3a 已完成离线指标；M4.3b 真实观察等待凭据。
