...
> 当前切片：L2/M5.1 已完成 `pa.persona` 单模板物理迁移、内容/组装摘要复核和本地/远端
> 全部门禁；下一批继续迁移剩余 28 个模板。
...
| L2 Prompt 模板引擎 | Prompt ID M1-M4.3、退出门禁、M5.0 和 M5.1 Persona 单模板迁移已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、Stage 1 schema v2、router 权威兼容 Normalizer、manifest v2、无 filename 模型上下文、固定 `tiktoken 0.12.0`、离线/live 退出报告、source path 投影和首个 `.prompt.md` 样本已交付 | 分批迁移剩余 28 个模板；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| 下一轮 | L2/M5.2 | `pa.persona` 已 `git mv` 为 `.prompt.md`，legacy filename、双口径正文摘要、M4 Prompt metrics 和四种组装摘要不变；run `30474245087` 双矩阵通过 | 按依赖边界设计下一迁移批次，继续保持每批路径变更、摘要证据、原子提交和远端验收 |
