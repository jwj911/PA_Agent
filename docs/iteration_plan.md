...
> 最近验收：GitHub Actions run `30490054789` 在 `main@980c294` 的
> Windows/Python 3.11/3.12 双矩阵全部通过。
> 当前切片：L2/M5.2 已完成 `pa.binary_decision` 单模板物理迁移、摘要复核和本地/远端
> 全部门禁；下一批继续迁移剩余 27 个模板。
...
| 下一轮 | L2/M5.3 | `pa.binary_decision` 已 `git mv` 为 `.prompt.md`，全部摘要和决策树合同不变；run `30490054789` 双矩阵通过 | 按依赖边界迁移 Stage 1/Base 模板，继续保持每批路径变更、摘要证据、原子提交和远端验收 |
...
3. **L2/M5：继续分批执行 `.prompt.md` 物理存储迁移**。每批只允许修改 manifest
   ...
   Prompt digest 不变。M5.2 已单独迁移共享 `pa.binary_decision` 并验证决策树解析合同。
