...
> 最近验收：GitHub Actions run `30474245087` 在 `main@fe119af` 的
> Windows/Python 3.11/3.12 双矩阵全部通过。
> 当前切片：L2/M5.2 已完成 `pa.binary_decision` 单模板物理迁移和内容/组装/决策树摘要
> 复核，正在执行完整本地质量门禁与远端验收。
...
| L2 Prompt 模板引擎 | Prompt ID M1-M4.3、退出门禁、M5.0 和 M5.1 已完成；M5.2 Binary Decision 单模板迁移正在验收；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、Stage 1 schema v2、router 权威兼容 Normalizer、manifest v2、无 filename 模型上下文、固定 `tiktoken 0.12.0`、离线/live 退出报告、source path 投影和两个 `.prompt.md` 样本已交付 | M5.2 验收后分批迁移剩余 27 个模板；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| 进行中 | L2/M5.2 | `pa.binary_decision` 已 `git mv` 为 `.prompt.md`，legacy filename、双口径正文摘要、M4 Prompt metrics、四种组装摘要和决策树解析合同不变 | 完成完整本地门禁、原子提交和 GitHub Actions Python 3.11/3.12 验收后再启动依赖模板批次 |

当前外部证据链路只剩 **L5 真实 outcome/evidence**。L2/M4.3 的离线与真实 Provider 门禁
均已完成，M5 不再受 live 证据阻塞；L3/L6 已收口并进入持续观察，L1 的实现与下线策略按
政策 retain，L4 已进入每日观察。
...
1. **L2/M5.0：先收口 Source Path 兼容准备**。M4.3 全部门禁已通过；诊断
   ...
   仍可用，再独立提交。该切片已完成。
2. **L2/M5.1：以 `pa.persona` 启动物理迁移**。首批只改变物理路径和 manifest
   `source_path`，已证明 legacy filename、双口径正文摘要、M4 Prompt metrics 和四种
   assembled Prompt digest 不变；本地/远端全部门禁已通过，该切片已收口。
3. **L2/M5：继续分批执行 `.prompt.md` 物理存储迁移**。每批只允许修改 manifest
   `source_path` 与物理路径，必须保持 Prompt ID、legacy filename、正文 SHA-256 和 assembled
   Prompt digest 不变。M5.2 先单独迁移共享 `pa.binary_decision`，并额外验证决策树解析合同。
...
7. **L1/L2 旧入口**：按 `compatibility_policy.json` retain；证据与版本条件未满足前不删除。
...
显示名/ID/tooltip。M1-M3 的 Prompt 字节保持不变。M4 模型合同清理必须另开评估切片，并在
进入 M5 前完成离线与真实 Provider 对照。M4.1 已完成 schema/Normalizer 边界，M4.2 已完成
Prompt/carryover/golden 和 manifest v2，M4.3a 已完成离线指标和 M3-compatible 真实基线；
M3/M4 token 比较已固定 `tiktoken 0.12.0` 并由依赖/基线一致性测试保护。M4.3b 候选
legacy/Pipeline 已通过 comparator，最终报告为 `m4_exit_gate_passed=true`；下一切片进入 M5。
...
## 2.24.2 本轮完成结果（L5：分析记录人工 outcome 导入）

- 新增 `pa_agent.records.experience_curation`，对 `records/pending/**/*.json` 做 shape-only
  eligibility scan；输出只包含计数、cycle 分布和稳定拒绝原因。
- 新增 `tools/curate_experience_record.py scan|import-record`；只有完整 Stage 1/2、合法
  cycle/direction/patterns 和至少 3 根 OHLC 的记录可导入，partial/损坏/缺字段记录显式拒绝。
- `import-record` 强制人工提供 `success|failure`；不得从 AI 置信度、是否下单、
  `terminal_outcome` 或预测自动推断真实交易结果。
- 导入内容裁剪为 meta、cycle、direction、patterns、K 线和结构化 Stage 1/2，不保留源路径/
  文件名、Prompt、Provider 原始回复、usage、策略路径或 HTF 原文；当前 API Key 再次递归脱敏。
- 内容 digest 和无 symbol 文件名保证重复导入幂等；同一记录改判 outcome 会冲突失败。
...
## 2.24.4 本轮完成结果（L5：脱敏 record review catalog）

- 新增 `pa-agent.experience-curation-review.v1`，为 eligible records 生成与源文件名无关的稳定
  `record_id`、catalog digest 和聚合拒绝原因；
- catalog 的单条字段严格限制为 timestamp、timeframe、cycle、direction、pattern 数量和
  `record_id`，不写 symbol、价格、K 线、Prompt、Provider 回复、文件名或路径；
- CLI 新增 `export-review`，`import-record` 支持 `--record-id` + `--records-dir`；按 ID 导入
  仍复用显式 outcome、最小化裁剪、Key 脱敏、幂等和冲突门禁；
- 文件重命名稳定性、安全输出、未知 ID 拒绝和 CLI 路径均有测试；L5/质量门禁测试
...
## 2.24.5 本轮完成结果（L5：可核验 outcome evidence）

- 新增 `pa-agent.outcome-evidence.v1`，固定 `realized-net-pnl-sign.v1`：已平仓净收益正/负分别
  映射 success/failure；未成交、未平仓、盈亏为 0 或证据不明继续 defer；
- CLI `import-record` 强制非空 `--evidence-file` 及 allowlist `--evidence-type`，只保存 evidence
  SHA-256、policy 和 type，不复制文件名、路径、symbol、价格、PnL 或正文；
- record digest 排除 outcome/evidence provenance，使记录身份稳定，同时由既有冲突门禁拒绝
  改判 outcome 或替换 evidence；
- 运行手册新增不同 `meta.symbol` 的第二 instrument group GUI/headless 生成步骤；当前记录若
  继续 defer，必须收集两个不同 symbol 的新案例；
...
## 2.24.6 本轮完成结果（L5：evidence-backed evaluator 门禁）

- curation 的 evidence validator 提升为共享公开合同，离线 catalog loader 对每个案例校验
  schema、固定 policy、allowlist type 和 SHA-256；
- `export-labels`、readiness preflight 和 `evaluate` 共用该 catalog loader，缺失或篡改
  evidence 时稳定拒绝，不能通过 Python API 或旧案例绕过 CLI；
- 在线 `ExperienceReader`、相似度排序和 legacy fallback 不变，本轮只收紧离线真实评估输入；
- 正向 fixtures 全部带合法 evidence，新增缺失 evidence、非法 digest、脱敏 readiness blocker
  和 export 拒绝测试；evaluator 直接相关测试 **21 passed**，含 reader/baseline/CI policy 的
  聚焦质量门禁 **41 passed**，Focused Ruff、Ruff format、`py_compile` 和差异检查通过；
- 当前 `experience/` 仍为 0 个 JSON，本轮没有导入合成案例或生成虚假真实指标。
- GitHub Actions run `30160388537` 已对 `main@b963cee` 完成 Windows/Python 3.11/3.12
  双矩阵验收，两个 job 均为 `success`。

## 2.24.7 本轮完成结果（L5：本地 outcome evidence 重核门禁）

- 将非空 evidence 文件 SHA-256 计算提升为 curation/evaluator 共享合同，导入与离线重核使用
  相同的不可读、空文件和分块读取语义；
- `preflight` 可接收 `--evidence-dir`，只输出 `evidence_status`、文件数、已核验案例数和稳定
  blocker；目录缺失、含空/不可读文件或案例摘要不匹配分别拒绝；
- `export-labels` 和 `evaluate` 强制要求 `--evidence-dir`，每次都重新读取本地文件；删除或
  替换导入时的原始证据后不能生成模板或报告；
- 报告新增 `outcome_evidence_revalidated=true`、catalog 引用的唯一 evidence digest 数和
  已核验案例数，不输出摘要值、文件名、路径、symbol、价格或正文；额外非空本地文件不会改变
  报告；
- 正向 fixtures 改为写入真实本地 evidence 字节并独立计算摘要；缺目录、替换文件、3/4 部分
  匹配和双入口拒绝均有测试。curation/evaluator 直接相关测试 **16 passed**，含
  reader/baseline/CI policy 的聚焦门禁 **36 passed**，Focused Ruff、Ruff format、
  `py_compile`、CI target 清单和差异检查通过；
- 当前真实 preflight 仍为 0 个经验案例，`evidence_status=not_checked`；没有生成合成经验或
  虚假指标，真实数据外部前置条件不变。
