...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1-M4.2 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、Stage 1 schema v2、router 权威兼容 Normalizer、manifest v2 和无 filename 模型上下文已交付 | 执行 M4.3 独立评估，再按证据决定 M5 后缀迁移；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1-M4.2 已完成；Stage 1 schema v2、manifest v2、无 filename 运行时 Prompt 和模型 carryover 已交付 | 下一切片为 M4.3：固定 fixture 下比较校验失败率、重试率、Token 和语义冲突率；具备会话级凭据时再执行真实 Provider 观察 |
...
1. **L2/M4.3：执行离线合同评估与真实 Provider 观察**。M4.2 已完成全量/增量 Prompt、
   Stage 2 carryover、manifest v2 和预期 golden 漂移；下一步比较校验失败率、重试率、
   Token 和语义冲突率。仅在存在会话级 `PA_AGENT_LIVE_API_KEY` 时执行真实 Provider 对照，
   不得伪造 live 证据。
...
Prompt/carryover/golden 和 manifest v2；M4.3 负责离线指标与真实观察。
