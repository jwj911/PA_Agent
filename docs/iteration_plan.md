...
> 当前代码/合同侧迭代和 L6 真实成功主路径已完成；未收尾项为 L3 重复稳定观察与 L5
> 真实数据证据。
...
| L3 Pipeline Builder | 首个真实 legacy/Pipeline pair 已通过，默认 legacy，重复稳定观察与默认切换未收口 | 完整四步 Pipeline、默认关闭的 flag、Task 10 终态矩阵、5 场景×3 轮 fixture 对照；2026-07-23 两次单体校验和成对校验均为 `valid=true` | 按同一合同继续重复观察；无未解释偏差后才评估默认 flag |
...
| P0 | L3 | 完整四步 Pipeline、默认关闭 flag、Task 10 矩阵、受控 rollout及首个真实成功 pair 已交付 | 重复真实 pair 无控制流/结构偏差后才评估启用默认 flag |
...
当前外部证据主链路为 **L3 → L5**；L6 真实成功主路径已收口并进入持续观察，L1/L2 的实现
与下线策略已收口并按政策 retain，L4 已进入每日观察，不再是当前阻塞项。
...
flag-off/flag-on 受控 fixture rollout 观察，并于 2026-07-23 取得首个真实 Provider
`valid=true` pair。默认 flag 仍关闭，后续必须完成重复稳定观察。

推荐顺序如下：

1. **L3：重复真实 Pipeline rollout 观察和默认开关评估**。
2. **L5：真实案例 opaque 导出、人工标注和离线指标报告**。
3. **L6**：保持持续观察；Provider、事件或记录合同变化时重跑真实 pair。
4. **L4**：每日持续基准，不凭单次波动修改热路径。
5. **L1/L2**：按 `compatibility_policy.json` retain；证据与版本条件未满足前不删除。
...
3. headless 与 GUI 已通过真实 `_AnalysisWorker`/`HeadlessAnalysisAdapter` 和固定 fixture
   ...
   策略文件回调等价；真实 Provider 成功主路径已建立，重复稳定观察仍待完成。
...
6. L3 的四个真实步骤已具备 opt-in 等价与失败路径证据：`PersistStep` 集中 full/partial
   ...
   首个真实成功 pair 已无控制流/结构偏差，但重复稳定观察与默认切换仍未收口；L6 的
...
`orchestrator.pipeline_builder_enabled` 默认仍为 `false`。
