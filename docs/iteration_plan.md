...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1、M2.1 Router 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID 与 manifest 映射、ID 路由/legacy 投影、TemplateContext、golden snapshots 和整组回退已交付 | 执行 M2.2 shared system 与 Stage 1 内部 ID 切换；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1 Catalog、M2.1 Router 双入口已完成；assembler、Pipeline 和 record 仍以 `.txt` 文件名传递 | 下一切片为 M2.2 shared system/Stage 1 内部 ID；旧 `*_txt_files()` 与全部 Prompt digest 必须不变 |
...
1. **L2/M2.2：shared system 与 Stage 1 内部切换到 ID**。M2.1 已完成
   `route_strategy_prompt_ids()` 和旧文件名等价投影；下一步切换 shared system/Stage 1，
   不改变 Prompt 正文、JSON schema 或 golden。
...
第 233 轮模板引擎迁移、Prompt ID M1 Catalog 和 M2.1 Router 双入口已完成；M2.2-M3
只做结构解耦并保持 Prompt 字节不变，不重复改写 L2 Prompt 文本。M4 模型合同清理必须另开
评估切片。
