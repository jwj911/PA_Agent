...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1-M2 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID 与 manifest 映射、ID 路由、Stage 1/2 原子 ID loader、TemplateContext ID/filename 双合同、golden snapshots 和整组回退已交付 | 执行 M3 Pipeline/record/GUI 双合同；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1-M2 已完成；Pipeline、record 和 GUI callback 仍以 `.txt` 文件名传递 | 下一切片为 M3 Pipeline/record/GUI 双合同；新记录双写 ID 与 legacy filename，旧记录不改盘可读 |
...
1. **L2/M3：Pipeline、record 与 GUI 双合同**。M1-M2 已完成；下一步让 Pipeline state 和新
   record 承载 Prompt ID，同时保留旧 callback/字段和旧记录读取，不改变 Prompt 正文或模型
   JSON schema。
...
第 233 轮模板引擎迁移和 Prompt ID M1-M2 已完成；M3 继续结构解耦并保持 Prompt 字节不变，
不重复改写 L2 Prompt 文本。M4 模型合同清理必须另开评估切片。
