...
| L6 Headless/编排 | headless 第一切片与公开 adapter 已交付，等价契约未收口 | `AppEvent`、`EventSink`、`JsonlEventSink`、`replay_jsonl`、`bootstrap_headless()`、共享 `_build_core()`、`bootstrap_gui()`、兼容 `bootstrap()`、`HeadlessAnalysisAdapter`、PyQt-free `pa-agent headless`；`analyze --run/--execute` 已接入两阶段 orchestrator、record 持久化和 JSONL 事件 | GUI/headless 最终/partial/cancel/failure record 等价、事件 envelope 版本契约和真实 Provider 环境验证 |
...
| 优先级 | 路线 | 当前阻塞项 | 收尾证据 |
|---|---|---|---|
| P0 | L6 | `headless analyze --run` 和公开 `HeadlessAnalysisAdapter` 已交付，但 GUI/headless 全链路等价和事件 envelope 版本契约仍缺 | mock Provider 下的 final/partial/cancel/failure record、事件重放与 GUI 对照；真实 Provider 只允许显式执行 |
...
2. **L6：GUI/headless 最终/partial/cancel/failure record 等价和事件 envelope 版本契约**。
3. **L5：脱敏经验数据集与离线评估基线**。
4. **L4：性能预算和持续基准**。
5. **L1 收口治理** 可以在主线空档穿插推进，但不得绕过 L6/L3 的等价证据。
6. **L2 兼容观察**：记录新旧路径运行结果，稳定一个周期后再单独评估旧 loader/helper 的下线。
7. **L1 治理观察**：registry 生命周期、replace/unregister、并发、lazy-import 和 entry point
   扩展契约已交付；后续只观察外部扩展兼容性和版本化契约策略。
...
6. L3 的四个真实步骤已具备 opt-in 等价与失败路径证据：`PersistStep` 集中 full/partial
   ...
   但真实稳定观察周期和 GUI/headless 全链路 final/partial/cancel/failure 等价仍未收口。L1 插件
   发现方案与正式扩展契约、L5 脱敏数据集与离线指标、L4 p50/p95 性能预算仍缺少固定证据，不能
   仅依据当前测试宣称收敛；L2 只剩兼容观察期。
...
- 固化 CLI 参数、record 摘要和事件 envelope 的版本边界，并补充公开
  `HeadlessAnalysisAdapter` 的 final/partial/cancel/failure 等价测试。
