...
| L2 Prompt 模板引擎 | 5 轮等价观察与下线策略/CI 门禁已收口；Prompt ID M1-M3 已完成；旧 loader/fallback 按政策 retain | TemplateStore、29 个稳定 Prompt ID、ID 路由/loader、TemplateContext/AnalysisRecord/Pipeline 双合同、GUI 显示名 + ID/tooltip、golden snapshots 和整组回退已交付 | 执行 M4 模型合同清理与独立评估，再按证据决定 M5 后缀迁移；最早 0.3.0 且满足 tag/fallback 零命中/golden 证据后才评估删除旧入口 |
...
| P1 | L2 | M1-M3 已完成；record/Pipeline 双写 ID/legacy filename，GUI 显示名称 + ID 且路径仅作 tooltip，旧 filename callback 保持兼容 | 下一切片为 M4：从新 Stage 1 模型合同移除 `strategy_files_needed`，router 成为唯一权威并执行离线/真实观察 |
...
1. **L2/M4：移除模型对 filename 的职责**。M1-M3 已完成；下一步从新 Stage 1 schema 与
   Prompt 合同中移除 `strategy_files_needed`，让已校验诊断经 router 唯一决定 Prompt ID；
   本阶段会有意改变 Prompt 字节，必须独立更新评估证据，旧 schema/记录继续兼容读取。
