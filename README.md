...
- 🔒 **API Key 至静态加密**：Windows 下经 DPAPI 加密后保存（密文绑定当前 Windows 账户，`settings.json` 泄漏亦无法解密）；非 Windows 平台优雅降级为明文保存。均置于被 `.gitignore` 忽略的本地配置，并配合 pre-commit 拦截与日志/记录运行时脱敏，避免密钥进入 Git 或日志
